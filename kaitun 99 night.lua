...
-- + Hop V2: tránh trùng server, tránh full, no-gems autohop (có grace)
-- + Stay windows: chống hop quá sớm (min ở lại sau join & sau viên đầu tiên)
...
local Config = {
    ...
    -- ====== Chống hop quá sớm ======
    MinStayAfterJoin        = 8.0,   -- (s) bắt buộc ở lại sau khi vừa join server
    MinStayAfterFirstGem    = 5.0,   -- (s) sau khi nhặt viên đầu tiên thì nấn ná thêm
    RequirePickupBeforeHop  = true,  -- chỉ cho phép HopFast nếu đã nhặt được gems

    -- ====== “No-gems / No-chest” bớt hung hăng ======
    NoGemsGrace             = 10.0,  -- (s) đợi gems spawn sau khi join trước khi kết luận no-gems
    NoChestScanWindow       = 15.0,  -- (s) thời gian quét chest trước khi kết luận no-chest
}
...
do
    ...
    -- (tuỳ chọn) lọc theo region nếu bật Config.RegionFilterEnabled
    ...
    function Hop(reason)
        ...
        local ok, why = canHopNow()
        if not ok then
            warn("[Hop] delayed by:", why)
            return
        end
    ...
    -- Mark JobId hiện tại sau khi tới (bảo hiểm) + reset session timer
    ...
    -- Sau khi vào server mới: nếu không thấy diamonds → hop (nhưng có grace)
            ...
            if inFarm() then
                if (Server.collectedCount or 0) == 0 then
                    ...
                    if aliveFor >= (Config.NoGemsGrace or 10) then
                        ...
                        local seen = false
                        while tick()-t0 < 4 do
                            if hasDiamonds() then seen = true; break end
                            task.wait(0.4)
                        end
                        if not seen then
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        ...
        if not chest then
            -- chỉ hop no-chest nếu đã quét đủ lâu kể từ khi join
