...
-- Skip servers từng fail (full/unavailable/…)
local BadIDs = {}
...
local function HopFast(reason)
    ...
    task.spawn(function()
        ...
        if not isTeleporting then
            ...
            local ok = pcall(function() _G._force_hop = true end)
            Hop()
        end
        HopRequested = false
    end)
...
-- ===== SERVER HOP (no rejoin same server, anti-dead-hop) =====
-- Bộ nhớ chống lặp theo giờ (persist toàn phiên)
do
...
local function tryTeleportOnce()
    ...
    for _, v in ipairs(page.data) do
        ...
        if playing and maxp and playing < maxp then
            -- Chặn: không vào JobId hiện tại, không vào server đã visited, và tôn trọng region/lists/bad
            if sid ~= game.JobId
            ...
            then
                ...
                isTeleporting   = true
                LastAttemptSID  = sid
                markVisited(sid)                -- đánh dấu ngay để tránh loop nếu API trả lại trùng
                resetState()                    -- dọn cache farm (không đụng visited)
                ...
                if not ok then
                    ...
                    isTeleporting   = false
                    BadIDs[sid]     = true        -- server lỗi → skip
                end
                return true
            end
        else
            BadIDs[sid] = true -- full trước khi xử lý
        end
    end
    return false
end
...
function Hop()
    ...
    -- Hết lựa chọn trang hiện tại → đảo hướng để mở phạm vi tìm
    cursor = ""
    fetchServerPage("", "Asc")
    task.wait(rand(1.0, 2.0))
end
...
TeleportService.TeleportInitFailed:Connect(function(_, teleportResult, msg)
    ...
    if LastAttemptSID then BadIDs[LastAttemptSID] = true end   -- ĐÁNH DẤU BAD; KHÔNG xoá visited
    isTeleporting = false
    HopRequested = false
    -- Cho phép Hop/FastHop gọi lại
    task.delay(0.75, Hop)
end)

Players.LocalPlayer.OnTeleport:Connect(function(state)
    if state == Enum.TeleportState.Started then
        -- Dọn cache farm khi bắt đầu rời server; KHÔNG xoá visited
        resetState()
    end
end)
