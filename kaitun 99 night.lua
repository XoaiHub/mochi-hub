...
local Config = {
    ...
    -- Giảm tỉ lệ hop vào server full (mặc định); sau-collect sẽ yêu cầu cao hơn
    MinFreeSlotsDefault     = 2,
    MinFreeSlotsCeil        = 4,

    -- Chọn kiểu server khi HopClassic
    ClassicMode             = "Low", -- "Low" = ưu tiên free slots; "High" = ưu tiên đông người
    ...
    -- Chống spam hop (vá)
    HopCooldownMin          = 0.25,  -- tối thiểu giữa 2 lần HopClassic
}
...
-- ====== SLOT LOGIC (NÉ FULL) ======
local function getEffectiveMinFreeSlots(reason)
    -- Sau khi nhặt xong (after-collect) => yêu cầu ≥3 slot để né full
    if reason == "after-collect" or reason == "after-normalchest-collect" or reason == "after-stronghold-collect" then
        return math.min(3, Config.MinFreeSlotsCeil or 4)
    end
    -- Khi TF/locked/anti-dead -> tăng nhẹ tạm thời
    if reason == "locked" or reason == "retry" or reason == "anti-dead" then
        return math.min((Config.MinFreeSlotsDefault or 2) + 1, Config.MinFreeSlotsCeil or 4)
    end
    -- Decay về mặc định khi hết TF lâu
...
-- Chọn phòng & Teleport (ưu tiên free slot + ping thấp nếu có)
local function chooseAndTeleportFromPage(siteData, mode, reason)
    if not siteData or not siteData.data then return false end
    local needFree = getEffectiveMinFreeSlots(reason)
    local serverList = {}
    ...
    for _, v in pairs(siteData.data) do
        ...
        if maxp > 0
           ...
           and regionMatch(v) then
            table.insert(serverList, { id=id, players=playing, free=free, ping=ping })
        end
    end
    ...
    -- Lấy 1 trong top 3 để tránh đụng nhau khi multi-tab
...
local function HopClassic(mode, reason)
    ...
    local bypass = false
    if reason == "after-collect" or reason == "after-normalchest-collect" or reason == "after-stronghold-collect"
       or reason == "anti-dead" or reason == "retry" or reason == "locked" then
        bypass = true
    end
    if not bypass then
    ...
    for _=1, Config.MaxPagesPrimary do
        ...
        if chooseAndTeleportFromPage(site, mode, reason) then HopLock = false return end
    ...
    HopLock = false
    task.wait(0.4 + math.random()*0.3) -- nhẹ CPU, tránh spam
end
...
local function HopFast(reason)
    ...
    task.spawn(function()
        ...
        if not isTeleporting then
            ...
            local why = tostring(reason or "after-collect")
