-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL (FIX LOBBY-CREATE SEEK)
...
-- + NEW: Ở LOBBY nếu KHÔNG có ai đang "create" → tự hop qua lobby khác tìm server có người đang create và JOIN vào create đó
--   (Giữ nguyên toàn bộ chức năng cũ ở map farm)
-- ================================================================
...
local Config = {
    ...
    -- Chọn kiểu server khi HopClassic ở FARM
    ...
    -- LOBBY seeker
    LobbySeekCooldown       = 2.0,   -- tránh spam hop lobby
    LobbyMode               = "Asc", -- tìm server lobby theo Asc trước rồi fallback Desc
}
...
-- Tải trang server (generic cho bất kỳ placeId)
local function fetchServerListGeneric(placeId, cursor, sort)
    ...
    local base = ("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=%s&excludeFullGames=true&limit=100")
                 :format(tostring(placeId), sortOrder)
...
-- Chọn phòng & Teleport (generic)
local function chooseAndTeleportFromPageGeneric(placeId, siteData, needFree, mode)
...
-- Fallback chọn chiều sort (generic)
local function HopGeneric(placeId, needFree, mode, maxPrimary, maxFallback, firstSort, secondSort, reason)
    ...
    local cursor, sort = "", (firstSort or "Asc")
    for _=1, (maxPrimary or Config.MaxPagesPrimary) do
        local site = fetchServerListGeneric(placeId, cursor, sort)
        if not site then break end
        if chooseAndTeleportFromPageGeneric(placeId, site, needFree, mode) then HopLock = false return end
        cursor = site.nextPageCursor or ""
        if not cursor or cursor == "" then break end
    end

    cursor, sort = "", (secondSort or "Desc")
    for _=1, (maxFallback or Config.MaxPagesFallback) do
        local site = fetchServerListGeneric(placeId, cursor, sort)
        if not site then break end
        if chooseAndTeleportFromPageGeneric(placeId, site, needFree, mode) then HopLock = false return end
        cursor = site.nextPageCursor or ""
...
-- Hop Classic cho FARM (giữ guard lobby)
local function HopClassic(mode, reason)
    ...
    HopGeneric(Config.FarmPlaceId, needFree, mode, Config.MaxPagesPrimary, Config.MaxPagesFallback, "Asc", (mode == "Low" and "Desc" or "Asc"), reason or "manual")
end

-- PUBLIC HOP API (farm)
local function Hop(reason_or_mode)
    local mode = (reason_or_mode == "High") and "High" or Config.ClassicMode
    HopClassic(mode, (reason_or_mode == "High" or reason_or_mode == "Low" or reason_or_mode == nil) and "manual" or tostring(reason_or_mode))
end
...
TeleportService.TeleportInitFailed:Connect(function(_, result, msg)
    ...
    -- Nếu đang ở farm → thử lại Hop farm; nếu ở lobby → thử Hop lobby
    task.delay(0.25, function()
        if g.PlaceId == Config.FarmPlaceId then
            ...
            -- lobby hop retry
            local needFree = getEffectiveMinFreeSlots()
            HopGeneric(g.PlaceId, needFree, "Low", Config.MaxPagesPrimary, Config.MaxPagesFallback, Config.LobbyMode, (Config.LobbyMode=="Asc" and "Desc" or "Asc"), "lobby-retry")
        end
    end)
end)
...
-- ====== LOBBY: Auto join/create + SEEKER HOP ======
local LastLobbySeekAt = 0

local function teleporterHasCreator(model)
    -- Xác định một "create" đang có người host: thường label Players hiển thị "1/4" hoặc "2/4"…
    local gui  = model:FindFirstChild("BillboardHolder")
    ...
    if lab and typeof(lab.Text) == "string" then
        ...
        if x and y and x >= 1 and x < y then
            return true, x, y
        end
    end
    return false
...
local function tryJoinAnyCreate()
    -- Duyệt các bệ Teleporter và dịch tới cái có x>=1
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj:IsA("Model") and (obj.Name=="Teleporter1" or obj.Name=="Teleporter2" or obj.Name=="Teleporter3") then
            local ok = teleporterHasCreator(obj)
            if ok then
...
local function autoJoinOrCreate()
    -- 1) Ưu tiên JOIN vào create có người (x>=1)
    if tryJoinAnyCreate() then return end

    -- 2) Nếu KHÔNG có ai đang create → dịch đại vào 1 teleporter (để tự create nếu game thiết kế vậy)
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj:IsA("Model") and (obj.Name=="Teleporter1" or obj.Name=="Teleporter2" or obj.Name=="Teleporter3") then
            local enter = obj:FindFirstChildWhichIsA("BasePart")
...
local function LobbyHopSeekCreate()
    -- Chỉ hop trong lobby để tìm phòng có người đang create
    if (os.clock() - LastLobbySeekAt) < Config.LobbySeekCooldown then return end
    LastLobbySeekAt = os.clock()

    local needFree = getEffectiveMinFreeSlots()
    -- Ở lobby, mode "Low" để ưu tiên phòng trống hơn → tăng khả năng có slot join create
    HopGeneric(g.PlaceId, needFree, "Low", Config.MaxPagesPrimary, Config.MaxPagesFallback, Config.LobbyMode, (Config.LobbyMode=="Asc" and "Desc" or "Asc"), "seek-create")
end

task.spawn(function()
    while task.wait(Config.LobbyCheckInterval) do
        if g.PlaceId ~= Config.FarmPlaceId then
            -- Ở lobby: nếu có ai đang create -> join; nếu không -> hop sang lobby khác tìm
            local joined = tryJoinAnyCreate()
            if not joined then
                LobbyHopSeekCreate()
            end
        end
