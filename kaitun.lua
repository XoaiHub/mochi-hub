...
-- + Hop V2.4 (hardened): tránh trùng, tránh full, no-gems autohop (có grace)
-- + Stay windows: chống hop quá sớm (ở lại sau join & sau viên đầu tiên)
-- + Re-exec safe: reset state/connection khi dán lại script
-- + NEW: RecentTried blacklist (chặn vừa-teleport xong là quay lại)
-- + NEW: Không dùng softRejoin về cùng job; luôn chọn job mới
-- + NEW: Bắt API đổi (games.roblox.com / roproxy; v1/v2)
-- ================================================================
...
local Config = {
    ...
    NoGemsGrace             = 10.0,  -- (s)
    NoChestScanWindow       = 15.0,  -- (s)

    -- Farm-first: chắc chắn “hút sạch rồi mới hop”
    FarmHoldAfterSpawn      = 2.5,   -- (s)
    FarmDrainHold           = 1.8,   -- (s)
...
ENV.MOCHI._cons  = {}
ENV.MOCHI._loops = {}
ENV.MOCHI._ver   = "hop-v2.4-hardened"
...
-- Proximity helper (thiếu trong bản trước)
local function pressPromptWithTimeout(prox, holdTime)
    ...
    while prox and prox.Parent and prox.Enabled and (os.clock() - t0) < (holdTime or 6) do
        if typeof(fireproximityprompt) == "function" then
            ...
            -- Giải pháp fallback (đẩy nhân vật vào prompt nếu 0-hold)
...
-- ===== FPS BOOST (giữ nguyên tính năng) =====
do
...
-- ===== VISITED + TRIED (HARDENED) =====
repeat task.wait() until g:IsLoaded()

-- Visited lâu hạn (3h) + yêu cầu tối thiểu slot trống
...
-- Tried ngắn hạn (chặn lặp tại chỗ) – giữ ~50 job gần nhất
...
local function pruneRecentTried()
    ...
    for _, id in ipairs(RT.queue) do
        ...
        if ts and (n - ts) <= (RT.ttl or 900) then
...
local function markTried(id)
    ...
    if not RT.map[id] then
        ...
        RT.map[id]   = true
        RT.stamp[id] = now()
        while #RT.queue > (RT.cap or 50) do
...
local ENDPOINTS = {
    -- Một số game chuyển giữa v1 và v2; ta thử cả hai
...
local function pickServer(placeId)
    ...
    -- Không có ứng viên -> dọn 12% visited cũ + nới min_free_slots tạm thời
    if #bestList == 0 then
        ...
        -- tạm thời yêu cầu slot trống cao hơn để tránh GameFull
...
local function bindTP()
    ...
    bindConn(TeleportService.TeleportInitFailed:Connect(function(_, result, msg)
        ...
        if r:find("GameFull") or (msg and msg:lower():find("requested experience is full")) then
            -- tăng yêu cầu slot trống để lần sau an toàn hơn
            G.min_free_slots = math.min((G.min_free_slots or 2) + 1, 3)
            task.delay(90, function() G.min_free_slots = 2 end)
        end
        -- Đánh dấu job vừa thử là "tried" để không quay lại ngay
...
local function tpToJob(jobId)
    ...
    -- nếu trong 12s không chuyển cảnh, bỏ bận
    task.delay(12, function() STATE.BusyTeleport=false end)
    return true
end

-- KHÔNG soft rejoin về cùng job nữa; thay bằng thử chọn job khác liên tiếp
...
local function _HopInternal()
    ...
    local ok = hopAttemptLoop(5)
    if not ok then
        -- cuối cùng bất đắc dĩ: rejoin place nhưng vẫn bị chặn quay về job hiện tại
        local pick = pickServer(Config.FarmPlaceId)
        if pick and pick.id and pick.id ~= g.JobId then tpToJob(pick.id) end
    end
end
...
queue_on_teleport_compat([[
    ...
    pcall(function()
        if game.JobId and game.JobId ~= "" then
            ...
            -- recent tried cũng tránh spam quay lại
            getgenv().RecentTried = getgenv().RecentTried or {queue={}, map={}, cap=50, ttl=900, stamp={}}
            local RT = getgenv().RecentTried
