...
function Gear.tryUpgradeBackpack()
    -- Use Capacity to reliably detect current backpack
    local capacityBag, capacityIdx = Gear.getBackpackFromCapacity()
    if not capacityBag then
        -- Fallback if CoreStats not available
    ...
    -- Sync local tracking with Capacity-based detection
    Gear._ownedBags = {}
    ...
    -- Equip the highest owned bag if not already
...
function Quest.handleFeedItemTask(questName)
    ...
    for _, t in ipairs(progress.tasks) do
        if not t.complete and t.remaining > 0 then
            ...
            -- First: try matching a specific item name in the task
            for _, item in ipairs(items) do
                if string.find(desc, string.lower(item.name)) or string.find(typ, string.lower(item.name)) then
                    ...
                    if have >= t.remaining then
                        ...
                        if #bees > 0 then
                            local bee = bees[math.random(1, #bees)]
                            print("[Feed] Feeding " .. t.remaining .. " " .. item.name .. " at [" .. bee.X .. "," .. bee.Y .. "]")
                            HiveManager.Feed(bee.X, bee.Y, item.remote, t.remaining)
                            task.wait(1)
                            return true
                        end
                    end
...
function Quest.doFarmLoop(fieldName, duration)
    ...
    while tick() - loopStart < duration do
        ...
        elseif Config.EnableMobAvoidance and MobManager.isMobNearby() then
            ...
            -- Walk to nearest token (priority first)
            ...
            if token and TokenManager.isToken(token) then
                ...
                -- No token, walk to random field position
