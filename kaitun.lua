...
local function getSlots()
    return #getBase():WaitForChild("AnimalPodiums", 5):GetChildren()
end
...
local function getTPTime(inputCFrame, fromCFrame)
    ...
    return (inputCFrame.Position - fromCFrame.Position).Magnitude / getgenv().Configcuttay["Settings"]["Speed"]
end
...
local function Tween(Pos, Time)
    ...
    local tween = game:GetService("TweenService"):Create(
		val, 
		TweenInfo.new((humanoidRootPart.Position - Pos.p).magnitude / getgenv().Settings.TPSpeed, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, 0, false, 0), 
		{Value = Pos}
	)
...
local function RemoteSell(animalId)
    ...
    if remote then
        ...
        warn("RemoteSell: Remote not found.")
    end
end
...
local function SellAnimal(Animal, Price)
    ...
    if not prompt then
        warn("SellAnimal: Could not find proximity prompt for animal:", Animal)
        return
    end
    local maxAttempts = 5
    local success = false
    ...
    if not success then
        ...
        print("SellAnimal: Successfully sold '"..tostring(Animal).."'.")
    end
end
...
local function AutoSpin()
    ...
    if tostring(spins) == "0" then
        game:GetService("ReplicatedStorage").Packages.Net["RE/CandyEventService/Spin"]:FireServer()
    end
end
...
local function safeRequest(reqTable, attempt)
        ...
        if nextItem.Animal and nextItem.Animal.Parent then
            ...
            if rootPart then
                ...
                if promptAttachment then
                    ...
                    if prompt then
                        PathfindTo2(rootPart.CFrame + rootPart.CFrame.lookVector * 31)
                        local success, err = pcall(function()
...
local function sendGlobalNotification(animalInfo)
    LRM_SEND_WEBHOOK(
        "https://discord.com/api/webhooks/1385846689012912229/mSlKtZK_V1ZHTb2YiG8MV_Nu2E6hslGCkEE3hmDgVPXqJdSkIar2Fw6km7a35vpE7NVk",
        {
            username = "Brainrot Notification - gg/",
            avatar_url = "https://github.com/user-attachments/assets/d46f45d6-e4cd-4b9e-a3fc-7e64fa94d8d0",
            embeds = {
                {
                    ...
                    thumbnail = {
                        url = "https://github.com/user-attachments/assets/d46f45d6-e4cd-4b9e-a3fc-7e64fa94d8d0"
                    },
                    fields = {
                        {
                            name = "🐾 Brainrot Name",
                    ...
                    footer = {
                        ...
                        icon_url = "https://github.com/user-attachments/assets/d46f45d6-e4cd-4b9e-a3fc-7e64fa94d8d0"
                    }
                }
            }
        }
...
local function TaskBuyAnimal(data)
    ...
    repeat
        ...
        local rootPart = data.Animal:FindFirstChild("RootPart")
        if not rootPart then
            break
        end
        local promptAttachment = rootPart:FindFirstChild("PromptAttachment")
        ...
        repeat wait()
            ...
            if GetDistance(playerRootPart.Position, rootPart.Position) < 5 then
            local pcallSuccess, err = pcall(function()
...
workspace.RenderedMovingAnimals.ChildAdded:Connect(function(v)
    local rootPart = v:WaitForChild("RootPart", 5)
...
local function PreLock()
    ...
    if LockRemain == 0 or (GetTPToLockTime + 6 ) > LockRemain then
        if not TPingTP then
            TPingTP = true 
            if getgenv().Configcuttay["Settings"]["Lock Priority"] then
                cancelTasks("SELL")
...
--[[ ====== BEGIN AUTO-BUY FIX PATCH (merged) ====== ]]>
--[[
===============================================================================
AUTO-BUY FIX PATCH (Drop-in) — keeps all old features, fixes BUY to work with
your config. Use it by pasting at the END of your original script, or run it
right after loading the original. It does NOT remove any existing features.
===============================================================================
What this patch does:
- Respect ["Lock Base"] = false (no more sneaky locking). Cleanly stops listeners.
- Fix Tween speed (uses Settings.Speed instead of a missing TPSpeed).
- Robust root part detection for animals (HumanoidRootPart / RootPart / any BasePart).
- Reliable prompt activation (hold + fireproximityprompt fallback).
- Prevent webhook spam if URL is empty.
- Keep BUY/SELL logic intact but unblocked; resumes BUY after SELL.
- Leaves other features untouched (collect, analytics, rebirth, etc.).
===============================================================================
]]
...
local function _getCfg()
    return getgenv() and getgenv().Configcuttay or {}
end

local function _getSettings()
    local cfg = _getCfg()
    return (cfg and cfg["Settings"]) or {}
end

local function _getMoveSpeed()
    local s = _getSettings()["Speed"]
    s = tonumber(s) or 50
    if s <= 0 then s = 50 end
    return s
end
...
-- Find a stable "root" to move to for animal models
...
-- Try proximity prompt robustly
local function _tryPrompt(prompt)
...
-- ===================== Fix Tween to use config speed =====================
-- If a Tween(Pos, Time) exists, override it to compute duration from Speed.
-- If not defined, we define a simple movement Tween compatible with the script.
do
    local old_Tween = rawget(_G, "Tween") or Tween

    _G.Teeny_Internal_currentTween = _G.Teeny_Internal_currentTween or nil
    _G.Teeny_Internal_val = _G.Teeny_Internal_val or nil

    Tween = function(Pos, _TimeIgnored)
        waitForCharacter()
        ...
        if _G.Teeny_Internal_currentTween then _G.Teeny_Internal_currentTween:Cancel() end
        if _G.Teeny_Internal_val then _G.Teeny_Internal_val:Destroy(); _G.Teeny_Internal_val = nil end

        _G.Teeny_Internal_val = Instance.new("CFrameValue")
        _G.Teeny_Internal_val.Value = hrp.CFrame

        local dist = (hrp.Position - Pos.Position).Magnitude
        local duration = dist / _getMoveSpeed()
        if duration ~= duration or duration == math.huge or duration <= 0 then
            duration = 0.5
        end

        local tween = TweenService:Create(
            _G.Teeny_Internal_val,
            TweenInfo.new(duration, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
            { Value = Pos }
        )
        _G.Teeny_Internal_currentTween = tween
        tween:Play()
        ...
        while not completed do
            ...
            if not hrp or not humanoid then
                if conn then conn:Disconnect(); conn = nil end
                if _G.Teeny_Internal_currentTween then _G.Teeny_Internal_currentTween:Cancel(); _G.Teeny_Internal_currentTween = nil end
                if _G.Teeny_Internal_val then _G.Teeny_Internal_val:Destroy(); _G.Teeny_Internal_val = nil end
                return
            end
            if humanoid.Sit == true then humanoid.Sit = false end
            if _G.Teeny_Internal_val then hrp.CFrame = _G.Teeny_Internal_val.Value end
            task.wait()
        end

        _G.Teeny_Internal_currentTween = nil
        if _G.Teeny_Internal_val then _G.Teeny_Internal_val:Destroy(); _G.Teeny_Internal_val = nil end
    end
end

-- ===================== BUY/SELL overrides (safe) =====================
-- The original script defines these globals; we override only if they exist.
-- We rely on the original TaskManager, shouldBuyAnimal, isStorageFull, getAnimalsToSell, etc.

-- Path helpers provided by original; if missing, provide minimal fallbacks
if typeof(PathfindTo2) ~= "function" then
    function PathfindTo2(destination)
        -- Minimal fallback: simple Tween if PathfindTo2 not defined
        if typeof(destination) == "Vector3" then
            Tween(CFrame.new(destination))
...
-- Ensure these globals exist to avoid nil errors
TaskManager = TaskManager or { priorities = { LOCK=1, SELL=2, BUY=3, COLLECT=4, WAIT=5 } }
TaskManager.taskQueue = TaskManager.taskQueue or {}
TaskManager.pausedTasks = TaskManager.pausedTasks or {}
TaskManager.currentTask = TaskManager.currentTask or nil

createTask = createTask or function(taskType, priority, action, data)
    return { type=taskType, priority=priority, action=action, data=data or {}, cancelled=false }
end
addTask = addTask or function(task) table.insert(TaskManager.taskQueue, task); return true end
processTasks = processTasks or function() end
pauseBuyTasks = pauseBuyTasks or function() end
resumeBuyTasks = resumeBuyTasks or function() end
cancelTasks = cancelTasks or function(_) end

-- Keep queue that original uses
BuyingQueue = BuyingQueue or {}
CurrentBuyingAnimal = CurrentBuyingAnimal or nil

-- Override BuyAnimal with robust root + prompt
do
    local _orig_BuyAnimal = BuyAnimal
    BuyAnimal = function(Animal, Info)
        if not Animal or not Animal.Parent then return end
        local rootPart = _findRootPart(Animal)
        if not rootPart then return end
        local prompt = rootPart:FindFirstChild("PromptAttachment")
            and rootPart.PromptAttachment:FindFirstChild("ProximityPrompt")
        if not prompt then return end
        ...
        if not CurrentBuyingAnimal then
            ...
            PathfindTo2(rootPart.CFrame + rootPart.CFrame.LookVector * 31)
            _tryPrompt(prompt)
        elseif (Info and CurrentBuyingAnimal and (Info.Price or 0) > (CurrentBuyingAnimal.Price or 0)) then
            table.insert(BuyingQueue, {Animal = Animal, Info = Info})
            PathfindTo2(rootPart.CFrame + rootPart.CFrame.LookVector * 31)
            _tryPrompt(prompt)
            CurrentBuyingAnimal = Info
            if typeof(processBuyingQueue) == "function" then
                processBuyingQueue()
            end
        else
            table.insert(BuyingQueue, {Animal = Animal, Info = Info})
...
-- Override TaskBuyAnimal to be resilient, but keep old SELL->BUY flow
do
    local _orig_TaskBuyAnimal = TaskBuyAnimal
    TaskBuyAnimal = function(data)
        -- Required external vars from original
        local CurrentCash = (Players.LocalPlayer.leaderstats and Players.LocalPlayer.leaderstats.Cash and Players.LocalPlayer.leaderstats.Cash.Value) or 0

        local initialCash = CurrentCash
        local Passed = false
        repeat
            -- refresh CurrentCash if available
            if Players.LocalPlayer.leaderstats and Players.LocalPlayer.leaderstats.Cash then
                CurrentCash = Players.LocalPlayer.leaderstats.Cash.Value
            end

            initialCash = CurrentCash
            local price = (data.Info and data.Info.Price) or math.huge
            local canAfford = CurrentCash >= price
            local shouldStillBuy = (typeof(shouldBuyAnimal) == "function") and shouldBuyAnimal(data.Info, data.GenPerSecond) or true
            local animalStillExists = data.Animal and data.Animal.Parent

            if Passed then shouldStillBuy = true end

            if not canAfford or not shouldStillBuy or not animalStillExists then
                -- print(("[BUY] cancel: afford=%s should=%s exists=%s"):format(tostring(canAfford), tostring(shouldStillBuy), tostring(animalStillExists)))
                break
            end

            if typeof(isStorageFull) == "function" and isStorageFull() then
                local sellableAnimals = (typeof(getAnimalsToSell) == "function") and getAnimalsToSell(data.Info) or {}
                if #sellableAnimals == 0 then break end
                local targetValue = (typeof(calculateTotalValue) == "function") and calculateTotalValue(data.Info) or math.huge
                local worstOwnedValue = (sellableAnimals[1] and sellableAnimals[1].value) or 0
                if targetValue <= worstOwnedValue then break end

                if typeof(pauseBuyTasks) == "function" then pauseBuyTasks() end
                Passed = true
                local sellTask = createTask("SELL", TaskManager.priorities.SELL, TaskSellAnimals, {targetBuyInfo = data.Info})
                addTask(sellTask)
                processTasks()
            ...
            if CurrentCash < price then break end
            if not data.Animal or not data.Animal.Parent then break end

            local rootPart = _findRootPart(data.Animal)
            ...
            -- Try to get close & activate prompt
            local tries, success = 10, false
            repeat
                task.wait()
                waitForCharacter()
                local character = LocalPlayer.Character
                ...
                if hrp then
                    ...
                    if (hrp.Position - rootPart.Position).Magnitude > 5 then
                        -- if far, path close
                        if typeof(PathfindTo2) == "function" then
                            PathfindTo2(rootPart.Position)
                        end
                    end
                end
                if hrp and (hrp.Position - rootPart.Position).Magnitude <= 6 then
                    success = _tryPrompt(prompt)
                    tries -= 1
            ...
            if not success then
                warn("[BUY] prompt failed after retries")
            end

            if TaskManager.currentTask and TaskManager.currentTask.cancelled then
                break
            end
        until (Players.LocalPlayer.leaderstats and Players.LocalPlayer.leaderstats.Cash and Players.LocalPlayer.leaderstats.Cash.Value or 0) < initialCash
    end
end

-- ===================== Respect Lock Base = false =====================
-- Stop AutoLock listeners when Lock Base is false; only run when true
_G.__AutoLockEnabled = _G.__AutoLockEnabled or false

local function StopAutoLock()
    _G.__AutoLockEnabled = false
    if rawget(_G, "LockLoop") and _G.LockLoop then
        pcall(function() _G.LockLoop:Disconnect() end)
        _G.LockLoop = nil
    end
end

do
    local _lastPreLockAt = 0
    local _preLockCooldown = 1.5

    local old_PreLock = PreLock
    PreLock = function()
        local s = _getSettings()
        if not (s and s["Lock Base"]) then
            return
        end
        if not _G.__AutoLockEnabled then return end

        local now = tick()
        if now - _lastPreLockAt < _preLockCooldown then return end
        _lastPreLockAt = now

        -- Fall back to original PreLock if present
        if typeof(old_PreLock) == "function" then
            old_PreLock()
            return
        end
    end

    local old_AutoLock = AutoLock
    AutoLock = function(boolen)
        StopAutoLock()
        if boolen then
            _G.__AutoLockEnabled = true
            -- If original AutoLock exists, delegate to it so other logic remains
            if typeof(old_AutoLock) == "function" then
                old_AutoLock(true)
            else
                -- If no original, wire a minimal listener if LockCheck exists
                local ok, LockCheck = pcall(function()
                    return getBase().Purchases.PlotBlock.Main.BillboardGui.RemainingTime
                end)
                if ok and LockCheck then
                    _G.LockLoop = LockCheck:GetPropertyChangedSignal("Text"):Connect(function()
                        if _G.__AutoLockEnabled then PreLock() end
                    end)
                    PreLock()
                end
            end
...
-- ===================== Mute empty webhook configs =====================
do
    local cfg = _getCfg()
    local wb = cfg["Webhook"]
    if wb then
        if wb.Enabled and (not wb.WebhookURL or wb.WebhookURL == "") then
            wb.Enabled = false
        end
        if wb.BuyNotificationSettings
           and wb.BuyNotificationSettings.Enabled
           and (not wb.BuyNotificationSettings.URL or wb.BuyNotificationSettings.URL == "") then
            wb.BuyNotificationSettings.Enabled = false
        end
    end
end

-- ===================== Apply AutoLock according to current config =======
do
    local wantLock = _getSettings()["Lock Base"] or false
    AutoLock(wantLock)  -- if false, listeners are stopped
end

-- ===================== End of BUY fix patch =====================

--[[ ====== END AUTO-BUY FIX PATCH ====== ]]
