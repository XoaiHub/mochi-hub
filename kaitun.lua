...
-- + Anti-Stuck + Watchdogs (no chest / no diamonds / no progress)
-- ================================================================
...
local Config = {
    ...
    -- Watchdogs bổ sung:
    NoChestHopTimeout     = 10.0,  -- không thấy chest usable đủ lâu -> hop
    NoDiamondHopTimeout   = 12.0,  -- không thấy diamond spawn/nhặt -> hop
    NoProgressHopTimeout  = 20.0,  -- không tăng tổng số diamonds đã bắn -> hop
    TeleportFailFallbacks = 3,     -- số lần fail liên tiếp trước khi fallback Teleport(place)
}
...
local ConsecutiveTpFails = 0

-- Watchdog states
local lastChestSeenAt      = os.clock()
local lastDiamondSeenAt    = os.clock()
local lastTotalDiamonds    = 0
local lastProgressAt       = os.clock()
local totalDiamondFires    = 0
            ...
            if playing and maxp and playing < maxp then
                if sid ~= localJob
                   ...
                   and regionMatch(v) then
                    ...
                    if not ok then
                        warn("[Hop] TeleportToPlaceInstance error:", err)
                        isTeleporting  = false
                        BadIDs[sid]    = true
                        ConsecutiveTpFails += 1
                        if ConsecutiveTpFails >= (Config.TeleportFailFallbacks or 3) then
                            -- Fallback: Teleport về Place (để Roblox tự chọn instance mới)
                            warn("[Hop] Fallback Teleport(place) after repeated fails")
                            isTeleporting = true
                            local ok2, err2 = pcall(function()
                                TeleportService:Teleport(PlaceID, LocalPlayer)
                            end)
                            if not ok2 then
                                warn("[Hop] Fallback error:", err2)
                                isTeleporting = false
                            end
                        end
                    end
...
local function findUsableChest()
    ...
    if seen then lastChestSeenAt = os.clock() end
    return closest
end
...
local function pressPromptWithTimeout(prompt, timeout)
    ...
    while prompt and prompt.Parent and prompt.Enabled and (tick()-t0) < (timeout or 6) do
        ...
        task.wait(0.28) -- hơi nhanh nhưng vẫn êm trên emulator
    end
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
                ...
                if prox and prox.Enabled then
                    ...
                    if not opened and (prox and prox.Parent and prox.Enabled) then
                        ...
                        if waitDiamonds(Config.StrongholdDiamondWait) then
                            ...
                            if got > 0 then
                                lastDiamondSeenAt = os.clock()
                                if totalDiamondFires > lastTotalDiamonds then
                                    lastTotalDiamonds = totalDiamondFires
        ...
        if not chest then
            -- nếu lâu không thấy chest usable -> watchdog hop
            if (os.clock() - lastChestSeenAt) >= Config.NoChestHopTimeout then
                HopFast("no-usable-chest")
            end
        else
...
task.spawn(function()
    while task.wait(Config.DiamondTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            -- Không thấy diamond quá lâu -> hop
            if (os.clock() - lastDiamondSeenAt) >= Config.NoDiamondHopTimeout then
                HopFast("no-diamond-seen")
            end
            -- Không tăng tổng số request lâu -> hop
            if (os.clock() - lastProgressAt) >= Config.NoProgressHopTimeout then
                HopFast("no-progress")
            end
        end
    end
