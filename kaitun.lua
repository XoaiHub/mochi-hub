-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL CHAIN HOTFIX
-- + Chain hop liên tục: nhặt xong là bay, vòng lặp không dừng
-- + Rương/Stronghold KHÔNG mở được -> hop ngay (không đứng yên)
-- + Anti-DEAD + Skip Server Full + No-Rejoin-Same-Server + Guard Lobby
-- + Hop engine: Asc/Desc + cursor (gia cố) + region filter (tùy chọn)
-- + Watchdogs: idle-no-diamond / teleport-stuck + Force-Hop Heartbeat
-- + API server list 3 lớp (v1 exclFull -> v1 -> v2) cho độ bền cao
...
local Config = {
    ...
    -- Chain hop liên tục
    ChainHopEnabled         = true,   -- nhặt xong là hop, lặp vô tận
    NoChestHopDelay         = 0.6,    -- không thấy rương usable -> hop nhanh
    IdleNoDiamondTimeout    = 10.0,   -- không có diamond quá lâu -> hop
    TeleportStuckTimeout    = 8.0,    -- teleport kẹt quá lâu -> reset & retry
    HopPostDelay            = 0.12,   -- giảm latency sau collect trước khi hop
    ...
    -- Giảm tỉ lệ hop vào server full (tự nâng yêu cầu slot tạm thời khi fail)
    ...
    JoinGraceSeconds        = 4.0,   -- ngay sau khi vào map farm
    FirstDiamondGrace       = 2.5,   -- ngay sau khi nhặt viên đầu (tránh bay quá sớm)
}
...
-- Chain hop state
local LastDiamondAt           = os.clock()
local LastTeleportAttemptAt   = 0
local LastJoinedServerAt      = os.clock()
...
local function tp_to_instance(placeId, serverId)
    LastTeleportAttemptAt = os.clock()
...
-- ===== SERVER LIST (bền bỉ 3 lớp) =====
local function fetchServerList(cursor, sort)
    local sortOrder = sort or "Asc"
    -- v1 excludeFull
    local base1 = ("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=%s&excludeFullGames=true&limit=100")
                  :format(tostring(Config.FarmPlaceId), sortOrder)
    local url1  = (cursor and cursor ~= "") ? (base1 .. "&cursor="..cursor) : base1
    url1 = url1 .. "&_t=" .. HttpService:GenerateGUID(false)

    local body = http_get(url1)
    local ok, data = (body and pcall(function() return HttpService:JSONDecode(body) end)) or false, nil
    data = ok and body and HttpService:JSONDecode(body) or nil
    if ok and data and data.data and #data.data > 0 then return data end

    -- v1 bình thường (không excludeFull)
    local base1b = ("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=%s&limit=100")
                   :format(tostring(Config.FarmPlaceId), sortOrder)
    local url1b  = (cursor and cursor ~= "") ? (base1b .. "&cursor="..cursor) : base1b
    url1b = url1b .. "&_t=" .. HttpService:GenerateGUID(false)
    body = http_get(url1b)
    ok, data = (body and pcall(function() return HttpService:JSONDecode(body) end)) or false, nil
    data = ok and body and HttpService:JSONDecode(body) or nil
    if ok and data and data.data and #data.data > 0 then return data end

    -- v2
    local base2 = ("https://games.roblox.com/v2/games/%s/servers/Public?sortOrder=%s&limit=100")
                  :format(tostring(Config.FarmPlaceId), sortOrder)
    local url2  = (cursor and cursor ~= "") ? (base2 .. "&cursor="..cursor) : base2
    url2 = url2 .. "&_t=" .. HttpService:GenerateGUID(false)
    body = http_get(url2)
    ok, data = (body and pcall(function() return HttpService:JSONDecode(body) end)) or false, nil
    data = ok and body and HttpService:JSONDecode(body) or nil
    if ok and data and data.data and #data.data > 0 then return data end
...
-- Lua 5.1 không có toán tử ? : , thay bằng function phụ
do
    local _old_fetch = fetchServerList
    fetchServerList = function(cursor, sort)
        local sortOrder = sort or "Asc"
        ...
        local base1 = ("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=%s&excludeFullGames=true&limit=100")
                      :format(tostring(Config.FarmPlaceId), sortOrder)
        local url1 = mk(base1) .. "&_t=" .. HttpService:GenerateGUID(false)
        local body = http_get(url1)
        local ok, data = (body and pcall(function() return HttpService:JSONDecode(body) end)) or false, nil
        data = ok and body and HttpService:JSONDecode(body) or nil
        if ok and data and data.data and #data.data > 0 then return data end
        ...
        local base1b = ("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=%s&limit=100")
                       :format(tostring(Config.FarmPlaceId), sortOrder)
        local url1b = mk(base1b) .. "&_t=" .. HttpService:GenerateGUID(false)
        body = http_get(url1b)
        ok, data = (body and pcall(function() return HttpService:JSONDecode(body) end)) or false, nil
        data = ok and body and HttpService:JSONDecode(body) or nil
        if ok and data and data.data and #data.data > 0 then return data end
        ...
        local base2 = ("https://games.roblox.com/v2/games/%s/servers/Public?sortOrder=%s&limit=100")
                      :format(tostring(Config.FarmPlaceId), sortOrder)
        local url2 = mk(base2) .. "&_t=" .. HttpService:GenerateGUID(false)
        body = http_get(url2)
        ok, data = (body and pcall(function() return HttpService:JSONDecode(body) end)) or false, nil
        data = ok and body and HttpService:JSONDecode(body) or nil
        if ok and data and data.data and #data.data > 0 then return data end
...
-- Chọn phòng & Teleport (nới lọc hợp lý)
local function chooseAndTeleportFromPage(siteData, mode)
    ...
    for _, v in pairs(siteData.data) do
        ...
        local maxp = tonumber(v.maxPlayers or v.maxPlayerCount or v.maxPlayerCapacity or 0) or 0
        local playing = tonumber(v.playing or v.playerCount or v.playerCountCurrent or 0) or 0
        local free = maxp - playing
        local passFree = (mode == "High") and (free >= 0) or (free >= needFree)
        if maxp > 0
           and passFree
           and id ~= game.JobId
           and not BadIDs[id]
           ...
           and regionMatch(v) then
            table.insert(serverList, { id=id, players=playing, free=free, ping=tonumber(v.ping or v.pingMs or 9999) })
        end
    end
    ...
    if mode == "Low" then
        ...
        local filtered = {}
        for _, s in ipairs(serverList) do if s.players >= 3 then table.insert(filtered, s) end end
        if #filtered > 0 then serverList = filtered end
    end

    if #serverList == 0 then
        DynamicMinFreeSlots = math.max(Config.MinFreeSlotsDefault - 1, 0)
        return false
    end

    if #serverList >= 3 then
        local i = math.random(1, math.min(3, #serverList)); serverList[1], serverList[i] = serverList[i], serverList[1]
    end
...
-- Hop Classic (Asc/Desc + multi pages + force retry)
local function HopClassic(mode, reason)
    ...
    -- Bypass grace cho các reason “hợp lệ”
    local bypass = false
    if reason == "after-collect"
    or reason == "anti-dead"
    or reason == "retry"
    or reason == "locked"
    or reason == "chain-collect-stronghold"
    or reason == "chain-collect-chest"
    or reason == "idle-no-diamond"
    or reason == "force" then
        bypass = true
    end
    if not bypass then
    ...
    for _=1, Config.MaxPagesPrimary do
        ...
        if chooseAndTeleportFromPage(site, mode) then jumped = true; break end
    ...
    -- Vẫn chưa nhảy được → ép gọi lại
    if not jumped then
        task.delay(0.75 + math.random()*0.5, function()
            if not isTeleporting and game.PlaceId == Config.FarmPlaceId then
                HopClassic(mode, "force")
            end
        end)
    end
end
...
-- Fast-hop sau collect / locked / idle
local function HopFast(reason)
    ...
    task.spawn(function()
        ...
        if not isTeleporting then
            ...
            HopClassic(Config.ClassicMode, reason or "after-collect")
        end
        HopRequested = false
    end)
end
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            if not chest then
                task.delay(Config.NoChestHopDelay, function()
                    if game.PlaceId == Config.FarmPlaceId and not isTeleporting then
                        Hop("Low")
                    end
                end)
            else
...
-- Watchdog: không thấy kim cương quá lâu -> hop
task.spawn(function()
    while task.wait(0.5) do
        if game.PlaceId == Config.FarmPlaceId and Config.ChainHopEnabled then
            if (os.clock() - LastDiamondAt) >= Config.IdleNoDiamondTimeout then
                HopFast("idle-no-diamond")
            end
        end
    end
...
-- Watchdog: teleport kẹt quá lâu -> reset & retry
task.spawn(function()
    while task.wait(0.5) do
        if isTeleporting and (os.clock() - LastTeleportAttemptAt) > Config.TeleportStuckTimeout then
            warn("[Hop] Teleport seems stuck, resetting flag and retrying...")
            isTeleporting = false
...
-- Force-Hop Heartbeat: nếu chain bật và không có diamond lâu -> ép hop nhịp nhàng
task.spawn(function()
    while task.wait(2.0) do
        if game.PlaceId == Config.FarmPlaceId and not isTeleporting then
            if Config.ChainHopEnabled and (os.clock() - LastDiamondAt) > math.random(8,12) then
                HopClassic(Config.ClassicMode, "idle-no-diamond")
            end
        end
    end
end)
