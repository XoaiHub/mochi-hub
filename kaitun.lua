...
local function regionMatch(serverEntry)
    if not Config.RegionFilterEnabled then
        return true
    end
...
local function tryTeleportOnce()
    ...
    cursor = (page.nextPageCursor and page.nextPageCursor ~= 'null')
            and page.nextPageCursor
        or ''
    for _, v in ipairs(page.data) do
        ...
        then
            if not hasValue(AllIDs, sid) and regionMatch(v) then
                ...
                warn(
                    ('Teleport -> %s (%s/%s)'):format(
                        sid,
                        tostring(v.playing),
                        tostring(v.maxPlayers)
                    )
                )
                isTeleporting = true
                pcall(function()
...
task.spawn(function()
    while task.wait(0.2) do
        local diamondGui = game:GetService('Players').LocalPlayer
...
local function checkTeleporter(obj)
    ...
    if
        g
        and g:FindFirstChild('BillboardGui')
        and g.BillboardGui:FindFirstChild('Players')
    then
    ...
    while task.wait(0.5) do
        for _, obj in ipairs(workspace:GetChildren()) do
            if
                obj:IsA('Model')
                and (
                    obj.Name == 'Teleporter1'
                    or obj.Name == 'Teleporter2'
                    or obj.Name == 'Teleporter3'
                )
            then
                checkTeleporter(obj)
            end
...
local function L_V3()
    ...
    for _, v in pairs(workspace.Items:GetDescendants()) do
        if
            ...
            and not v.Name:find('Snow')
        then
            local prox = v:FindFirstChild('ProximityInteraction', true)
            ...
            if prox then
                ...
                if tick() - chestSeen[id] <= 10 then
                    ...
                    if
                        ...
                        or not FogSize
                        or not (
                            p >= FogCF.Position - FogSize / 2
                            and p <= FogCF.Position + FogSize / 2
                        )
                    then
...
local function L_V4()
    for _, d in pairs(workspace:GetDescendants()) do
        if
            d:IsA('Model')
            and d.Name == 'Diamond'
            and game.PlaceId == 126509999114328
        then
    ...
    while task.wait(0.1) do
        if game.PlaceId == 126509999114328 then
            ...
            for _, v in pairs(workspace.Items:GetChildren()) do
                if
                    v.Name:find('Chest')
                    and v:FindFirstChildWhichIsA('ProximityPrompt', true)
                    and not v.Name:find('Snow')
                then
                    local p = v:GetPivot().Position
                    if
            ...
            if
                c < 0
                and not workspace:FindFirstChild('Diamond', true)
                and game.PlaceId == 126509999114328
            then
            ...
            while true do
                ...
                warn(
                    '[ChestFarm] Farming chest:',
                    chest and chest.Name or 'nil'
                )
                local prox =
                    chest:FindFirstChildWhichIsA('ProximityPrompt', true)
                local start = os.time()
                ...
                if os.time() - start >= 10 then
                    warn(
                        '[ChestFarm] Chest timeout (10s), skipping:',
                        chest.Name
                    )
                end
                task.wait(0.5)
            ...
            then
                ...
                for _, pl in pairs(Players:GetPlayers()) do
                    ...
                    then
                        if
                            pl.Character.HumanoidRootPart.Velocity.Magnitude > 2
                        then
                            inactive = false
                            break
                        end
