            ...
            if holder then
                ...
                if gui then
                    ...
                    if txt and txt.Text:lower():find("create") then
                        local prompt = tele:FindFirstChildWhichIsA("ProximityPrompt", true)
                        local entryPart = tele:FindFirstChildWhichIsA("BasePart")
                        if prompt and entryPart and lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
                            lp.Character:PivotTo(entryPart.CFrame + Vector3.new(0,3,0))
                            task.wait(0.3)
                            pcall(function() fireproximityprompt(prompt) end)
                            warn("[Lobby] Tìm thấy Create → Join ngay")
                            return
                        end
                    end
                end
