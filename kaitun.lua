-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL (v9.4)
...
-- + Stronghold/Chest locked -> Hop ngay
-- + Watchdog TeleportStart + Hop heartbeat + HTTP fallback + BadIDs TTL
...
local Config = {
    ...
    ClassicMode             = "Low", -- "Low" / "High"

    JoinGraceSeconds        = 4.0,
    FirstDiamondGrace       = 2.5,

    -- giữ vòng lặp luôn chạy
    ForceHopIfNoProgress    = 25,    -- không có tiến trình (mở rương/nhặt kim cương) -> hop
    MaxServerStaySeconds    = 120,   -- ở 1 server quá lâu -> hop
    RevisitDecaySeconds     = 4*3600,

    -- NEW: watchdog teleport
    TeleportStartTimeout    = 7.0,   -- gọi teleport mà không rời server trong ngần này giây -> xem là fail
    HopHeartbeatInterval    = 8.0,   -- nhịp ép gọi hop scan (bỏ qua grace) để không bao giờ đứng yên
}
...
-- HTTP getter tương thích (gia cố)
...
-- Grace windows
local JoinedFarmAt, FirstDiamondAt = 0, 0
...
-- Progress tracking
local LastDiamondTime, LastChestActionTime = 0, 0
local function touchProgress() LastChestActionTime = os.clock() end
local function touchDiamond() LastDiamondTime = os.clock() end
local function secondsSinceProgress()
    local last = math.max(LastDiamondTime, LastChestActionTime, JoinedFarmAt)
    if last <= 0 then return 1e9 end
    return os.clock() - last
end

-- ===== Teleport wrapper + WATCHDOG =====
local ConsecutiveHopFail, LastHopFailAt = 0, 0
local BadIDs = {} -- id -> timeMarked
...
local function pruneBadIDs()
    local now = os.clock()
    for id, t in pairs(BadIDs) do
        if (now - t) > Config.RevisitDecaySeconds then
            BadIDs[id] = nil
        end
    end
end

-- Chặn "Teleport Failed" UI
...
local function fetchServerList(cursor, sort)
    ...
    local url  = (cursor and cursor ~= "") ? (base .. "&cursor="..cursor) : base
    -- Lua không có toán tử ? :, dùng if:
    if cursor and cursor ~= "" then url = base .. "&cursor="..cursor else url = base end
...
-- Choose & Teleport (có watchdog TeleportStart)
local function tp_to_instance(placeId, serverId)
    ...
    if not ok then
        ...
        if not ok2 then
            warn("[TP] both methods failed:", err, err2)
            return false
        end
    end

    -- Watchdog: nếu không Start trong timeout -> coi là fail
    local t0 = os.clock()
    while (os.clock() - t0) < Config.TeleportStartTimeout do
        if TeleportStartedFlag then return true end
        task.wait(0.15)
    end
    warn("[TP] Start timeout -> treating as fail")
    return false
end
...
local function HopClassic(mode, reason)
    ...
    -- chỉ hop khi ở map farm (giữ nguyên guard)
    if g.PlaceId ~= Config.FarmPlaceId then return end

    -- bỏ qua grace cho các lý do an toàn
    local bypass = {
        ["after-collect"]=true, ["anti-dead"]=true, ["retry"]=true,
        ["locked"]=true, ["no-chest"]=true, ["no-progress"]=true,
        ["keepalive"]=true, ["scan"]=true
    }
    if not bypass[tostring(reason or "manual")] then
...
-- Sự kiện TeleportInitFailed (giữ nguyên + đánh dấu BadIDs)
TeleportService.TeleportInitFailed:Connect(function(_, result, msg)
    ...
    if LastAttemptSID then BadIDs[LastAttemptSID] = os.clock() end
...
local function collectAllDiamonds()
    ...
    if n > 0 then
        if FirstDiamondAt == 0 then FirstDiamondAt = os.clock() end
        touchDiamond()
    end
    return n
end
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            if not chest then
                HopClassic(Config.ClassicMode, "no-chest")
            else
...
task.spawn(function()
    while task.wait(Config.DiamondTick) do
        if g.PlaceId == Config.FarmPlaceId then
            if collectAllDiamonds() > 0 then touchDiamond() end
        end
    end
end)

-- ===== WATCHDOG + HEARTBEAT: BẢO ĐẢM LUÔN HOP =====
task.spawn(function()
    while task.wait(1.0) do
        if g.PlaceId == Config.FarmPlaceId then
            if secondsSinceProgress() >= Config.ForceHopIfNoProgress and not isTeleporting then
                HopFast("no-progress")
            end
            if (os.clock() - ServerJoinTime) >= Config.MaxServerStaySeconds and not isTeleporting then
                HopFast("keepalive")
            end
        end
    end
...
task.spawn(function()
    while task.wait(Config.HopHeartbeatInterval) do
        if g.PlaceId == Config.FarmPlaceId and not isTeleporting then
            -- Nhịp “scan” để không bao giờ đứng yên dù logic trên bỏ sót
            HopClassic(Config.ClassicMode, "scan")
        end
    end
end)
