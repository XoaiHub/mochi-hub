...
--// ================== SERVER HOP (chống dead-hop) ==================
local PlaceID = g.PlaceId
local AllIDs, cursor, isTeleporting = {}, "", false
local strongholdTried, chestTried = {}, {}  -- (đặt sớm để resetState sử dụng)
...
local function resetState()
    ...
    warn("[Hop] Reset state khi hop server mới.")
end
        ...
        if tonumber(v.playing) and tonumber(v.maxPlayers) and tonumber(v.playing) < tonumber(v.maxPlayers) then
            if not hasValue(AllIDs, sid) and regionMatch(v) then
                ...
                local ok, err = pcall(function()
                    resetState() -- reset cache trước khi chuyển server
...
function Hop()
    ...
    cursor = "" -- reset và đổi hướng tìm
    fetchServerPage("", "Asc")
    warn("[Hop] Không tìm thấy server, thử lại với sortOrder khác.")
end

-- Retry nếu teleport init fail
TeleportService.TeleportInitFailed:Connect(function(player, teleportResult, errorMessage)
    warn("[Hop] TeleportInitFailed -> "..tostring(errorMessage).." | retry...")
    isTeleporting = false
    task.delay(2, Hop)
end)

-- Reset state khi bắt đầu teleport (khi vào server mới client sẽ reload)
            ...
            if not strongholdTried[sid] then
                ...
                if prox and prox.Enabled then
                    ...
                    if not opened and (prox and prox.Parent and prox.Enabled) then
                        -- vẫn còn enabled => xem như khoá/bug -> skip vĩnh viễn
                        ...
                        if waitDiamonds(Config.StrongholdDiamondWait) then
                            ...
                            StrongholdCount += 1
                            strongholdTried[sid] = true -- đánh dấu đã xử lý xong
                            warn("[Stronghold] Đã nhặt diamond:", got)
                        else
                            -- không spawn diamond -> cũng skip vĩnh viễn
                            strongholdTried[sid] = true
