...
local Config = {
    ...
    -- Farm ưu tiên Stronghold trước, sau đó chest thường
    StrongholdChestName   = "Stronghold Diamond Chest",
    StrongholdPromptTime  = 10,
    StrongholdDiamondWait = 20,

    -- Map farm (dùng để phân biệt lobby vs farm)
    FarmPlaceId           = 126509999114328, -- đổi nếu game bạn khác
}
...
--// ================== UI (kiểu cũ full screen + counters) ==================
...
diamondLabel.TextStrokeTransparency = 0.6
...
normalLabel.TextStrokeTransparency = 0.6
...
strongholdLabel.TextStrokeTransparency = 0.6
...
local function pressPromptWithTimeout(prompt, timeout)
    ...
    -- trả true nếu prompt biến mất (đã trigger)
...
--// ================== MAIN FARM (ưu tiên Stronghold) ==================
spawn(function()
    while task.wait(1) do
        if g.PlaceId ~= Config.FarmPlaceId then continue end

        -- 1) ƯU TIÊN STRONGHOLD (nếu có và mở được)
        ...
        if stronghold then
            ...
            if prox and prox.Enabled then
                ...
                -- kiểm tra lại: nếu vẫn còn prompt enabled => khoá, bỏ qua
                ...
                -- không mở được -> bỏ qua, farm chest thường
                warn("[Farm] Stronghold khoá/không khả dụng -> bỏ qua")
            end
        end

        -- 2) FARM CHEST THƯỜNG (đến khi hết usable thì hop)
        while true do
            ...
            -- thành công nếu prompt biến mất hoặc disabled
...
-- 3) Diamonds farm song song (bao quát cả stronghold + chest thường)
spawn(function()
    while task.wait(0.2) do
        if g.PlaceId == Config.FarmPlaceId then
            collectAllDiamonds()
