...
-- ===== queue_on_teleport (nạp lại loader của bạn nếu cần) =====
        ...
        if playing and maxp and playing < maxp then
            if not BadIDs[sid] and not AllIDs[sid] and regionMatch(v) then
                ...
                if not ok then
                    ...
                    isTeleporting = false
                    BadIDs[sid] = true         -- đánh dấu bad
                end
                return true
            end
        else
            -- server đã full trước khi đến lượt -> bỏ qua
            BadIDs[sid] = true
        end
    end
    return false
...
TeleportService.TeleportInitFailed:Connect(function(_, teleportResult, msg)
    ...
    if LastAttemptSID then BadIDs[LastAttemptSID] = true end  -- nếu full/unavailable -> skip SID đó
    isTeleporting = false
    HopRequested = false
    task.delay(0.75, Hop)
end)
...
-- bind ngay hiện tại + những lần respawn
