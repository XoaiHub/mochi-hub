...
local function attemptToEnterFarmArea()
    for _, teleporter in ipairs(workspace:GetChildren()) do
        if teleporter:IsA("Model") and (teleporter.Name == "Teleporter1" or teleporter.Name == "Teleporter2" or teleporter.Name == "Teleporter3") then
            local billboardGui = teleporter:FindFirstChild("BillboardHolder.BillboardGui.Players", true)
            if billboardGui then
                local current, max = billboardGui.Text:match("(%d+)/(%d+)")
                current, max = tonumber(current), tonumber(max)
                if current and max and current >= 2 then
                    local entryPart = teleporter:FindFirstChildWhichIsA("BasePart")
                    if entryPart and lp.Character and lp.Character:FindFirstChild("Humanoid") then
                        local rootPart = lp.Character:FindFirstChild("HumanoidRootPart")
                        if rootPart then
                            if (rootPart.Position - entryPart.Position).Magnitude > 10 then
...
spawn(function()
    while task.wait(0.5) do
        if game.PlaceId~=126509999114328 then attemptToEnterFarmArea() end
    end
end)
