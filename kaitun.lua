...
-- + Hop V2 (stable): tránh trùng, tránh full, no-gems autohop (có grace)
...
local Config = {
    ...
    FarmHoldAfterSpawn      = 2.5,   -- (s) chờ gems xuất hiện sau khi mở rương
...
ENV.MOCHI._cons  = {}
ENV.MOCHI._loops = {}
ENV.MOCHI._ver   = "hop-v2.3-stable"
...
local function pickServer(placeId)
    ...
    -- Không có ứng viên -> dọn 10% visited cũ để tránh “cạn server”
    if #bestList == 0 then
...
-- Lấy vị trí thế giới của đối tượng
...
bindLoop("farm", function()
        ...
        if not strongholdTried[sid] then
            ...
            if prox and prox.Enabled then
                ...
                if not opened and (prox and prox.Parent and prox.Enabled) then
                    ...
                    -- Đợi gems spawn, hút sạch, rồi mới hop nếu có nhặt
    ...
    if not chest then
        ...
        if okOpen then
            -- Đợi spawn, hút sạch, rồi mới hop nếu có nhặt
            if waitDiamonds(Config.FarmHoldAfterSpawn or 2.5) then
                local before = Server.collectedCount or 0
                local got    = harvestDrain(Config.FarmDrainHold)
                local delta  = (Server.collectedCount or 0) - before
...
-- ===== NO-GEMS WATCHER (nhưng không đẩy đi nếu còn rương mở được) =====
