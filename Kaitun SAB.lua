...
local Config = {
    ...
    -- ===== NEW: RƯƠNG CHỈ MỞ 1 & 5 GEMS =====
    AllowedChestGemValues   = {1, 5},

    -- ===== NEW: LOBBY CREATE FINDER =====
    ...
    -- Khi đi săn lobby có người create, ưu tiên server có nhiều người
    LobbyPreferHighPlayers  = true,
}
...
local function scanLobbyTeleporterAndJoin()
                ...
                if x and y and x >= Config.MinPlayersOnCreate and x < y then
                    ...
                    if not joined then
                        ...
                        if enter and HRP() then
                            ...
                            joined = true -- join ngay cái có người trước
                        end
                    end
                end
            end
    ...
    if not joined then
        -- nếu chưa thấy ai create nhưng vẫn muốn tự create thì đứng vào bất kỳ teleporter
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj:IsA("Model") and (obj.Name=="Teleporter1" or obj.Name=="Teleporter2" or obj.Name=="Teleporter3") then
                local enter = obj:FindFirstChildWhichIsA("BasePart")
...
task.spawn(function()
    ...
    while task.wait(Config.LobbyCheckInterval) do
        if g.PlaceId ~= Config.FarmPlaceId then
            ...
            local joined, found = pcall(scanLobbyTeleporterAndJoin)
            joined, found = (joined and found ~= nil and (select(1, scanLobbyTeleporterAndJoin))) or false, (found == true)
            -- đơn giản hoá: chỉ kiểm tra timeout để hop
