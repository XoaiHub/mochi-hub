...
local function optimizeBatched(root, batchSize)
    batchSize = batchSize or 400
    local list = root:GetDescendants()
    local n = #list
    local i = 1
    while i <= n do
        local j = math.min(i+batchSize-1, n)
        for k=i,j do pcall(applyOptOne, list[k]) end
        i = j + 1
        task.wait()
    end
end

optimizeBatched(game, 400)
game.DescendantAdded:Connect(applyOptOne)
    ...
    for _, v in ipairs(page.data) do
        ...
        if tonumber(v.playing) and tonumber(v.maxPlayers) and tonumber(v.playing) < tonumber(v.maxPlayers) then
            if not hasValue(AllIDs, sid) and regionMatch(v) then
                ...
                isTeleporting = true
                local ok = pcall(function() TeleportService:TeleportToPlaceInstance(PlaceID, sid, LocalPlayer) end)
                if not ok then isTeleporting = false end
...
--// ================== STATE + CACHE ==================
local strongholdTried, chestTried, diamondSeen = {}, {}, {}
local LAST_SCAN, SCAN_COOLDOWN = 0, 0.5
...
local function findUsableChestCached()
    if tick() - LAST_SCAN < SCAN_COOLDOWN then return nil end
    LAST_SCAN = tick()
    ...
    for _, m in ipairs(items:GetChildren()) do
        if m:IsA("Model") and m.Name:find("Chest") and not m.Name:find("Snow") then
            local id = m:GetDebugId()
            if not chestTried[id] then
                local prox = m:FindFirstChildWhichIsA("ProximityPrompt", true)
                if prox and prox.Enabled then
                    local d = (hrp.Position - m:GetPivot().Position).Magnitude
                    if not dist or d < dist then closest, dist = m, d end
                end
            end
        end
    end
...
spawn(function()
    while task.wait(1) do
        ...
        -- CHEST THƯỜNG
        while true do
            local chest = findUsableChestCached()
            if not chest then
                Hop()
                break
            end
            ...
            if chestTried[id] then
                if not findUsableChestCached() then Hop() break end
                task.wait(0.4)
                continue
            end
