-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL CHAIN
-- + Fast Hop After Collect (nhặt xong là bay)
-- + Chain Hop liên tục (không dừng), rương/Stronghold khóa -> hop ngay
-- + Anti-DEAD + Skip Server Full + No-Rejoin-Same-Server + Guard Lobby
-- + Hop engine: Asc/Desc + cursor (gia cố) + region filter + grace windows
-- + Watchdogs: idle-no-diamond / teleport-stuck
...
local Config = {
    ...
    IdleNoDiamondTimeout    = 10.0,   -- không thấy diamond quá lâu -> hop
    TeleportStuckTimeout    = 8.0,    -- đang teleport quá lâu -> reset & retry
    ...
    FirstDiamondGrace       = 2.5,   -- sau khi nhặt viên kim cương đầu (tránh bay quá sớm)
}
...
local function tp_to_instance(placeId, serverId)
    -- đánh dấu thời điểm thử teleport để watchdog theo dõi
...
local function HopClassic(mode, reason)
    ...
    or reason == "idle-no-diamond" then
        bypass = true
    end
    if not bypass then
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            if not chest then
                -- không có rương usable -> delay rất ngắn rồi hop
