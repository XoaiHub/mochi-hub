...
-- + Stronghold: KHÔNG mở được trong StrongholdPromptTime (mặc định 6s) -> Hop ngay
-- + Stronghold: MỞ ĐƯỢC -> đợi spawn diamonds, NHẶT HẾT -> mới Hop
...
local Config = {
    ...
    StrongholdPromptTime    = 6,    -- KHÔNG mở được trong 6s -> Hop
    StrongholdDiamondWait   = 10,   -- sau khi mở, chờ diamonds tối đa 10s
    ...
    HopAfterStronghold      = true,  -- mở được stronghold & nhặt xong -> hop
    HopAfterNormalChest     = true,  -- chest thường: nhặt xong -> hop
    HopPostDelay            = 0.20,
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            -- 1) Stronghold: KHÔNG mở được trong 6s -> Hop; mở được -> nhặt hết rồi mới Hop
            ...
            if sh then
                ...
                if not strongholdTried[sid] then
                    ...
                    if prox and prox.Enabled then
                        ...
                        -- Bấm prompt trong StrongholdPromptTime (6s)
                        local opened = pressPromptWithTimeout(prox, Config.StrongholdPromptTime)
                        prox = findProximityPromptInChest(sh) -- refresh handle

                        if not opened and (prox and prox.Parent and prox.Enabled) then
                            -- Không mở được trong 6s => coi như LOCKED -> hop ngay
                            strongholdTried[sid] = true
                            HopFast("stronghold-locked")
                        else
                            -- Đã mở: chờ diamonds spawn, rồi nhặt sạch mới hop
                            if waitDiamonds(Config.StrongholdDiamondWait) then
                                ...
                                -- Mở nhưng không spawn diamonds trong thời hạn -> hop
                                strongholdTried[sid] = true
                                HopFast("stronghold-no-diamonds")
                            end
                        end
                    else
                        -- Không có prompt (hết lượt / cooldown) -> đánh dấu, bỏ qua server này
                        strongholdTried[sid] = true
                        HopFast("stronghold-missing-prompt")
                    end
                end
            end

            -- 2) Chest thường: không mở được (timeout) -> Hop ngay; mở được -> nhặt hết mới Hop
