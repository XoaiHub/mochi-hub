...
-- Bắt lỗi TeleportInitFailed (xử lý Unauthorized/Game 512 cho Wave)
TeleportService.TeleportInitFailed:Connect(function(_, result, msg)
    ...
    if LastAttemptSID then
        -- Unauthorized / Game 512 / RequestRejected → bỏ hẳn JobId
        if rstr:find("Unauthorized") or (msg and msg:lower():find("game 512"))
           or rstr:find("RequestRejected") then
            BadIDs[LastAttemptSID] = true
        else
            BadIDs[LastAttemptSID] = true
        end
    end
    ...
    ConsecutiveHopFail += 1
    LastHopFailAt = os.clock()
    task.delay(0.5, Hop) -- nhảy tiếp Job khác ngay, không retry Job cũ
end)
