...
local Config = {
    ...
    -- Giảm tỉ lệ hop vào server full (nâng yêu cầu slot khi bị full)
    MinFreeSlotsDefault     = 1,
    MinFreeSlotsCeil        = 3,

    -- Watchdog kẹt server
    WatchdogInterval        = 10,     -- mỗi 10s kiểm tra kẹt
    WatchdogNoLootTimeout   = 45,     -- >45s không có chest/diamond mới => hop
    PostTeleportVerifyWait  = 6,      -- sau Teleport, nếu vẫn cùng JobId sau 6s => coi như fail
}
...
local function http_get(url)
    ...
    if req then
        ...
        if ok2 and res2 and (res2.Success or (res2.StatusCode==200)) and type(res2.Body) == "string" then
            return res2.Body
        end
    end
    return nil
...
local function tp_to_instance(placeId, serverId)
    ...
    if ok then
        -- Post-teleport verify: nếu 6s sau vẫn ở cùng JobId => coi như fail
        task.delay(Config.PostTeleportVerifyWait, function()
            if g.JobId == beforeJob then
                warn("[TP] Post-verify: still in same JobId ⇒ treat as fail, sid=", tostring(serverId))
                BadIDs[serverId] = true
                isTeleporting = false
                ConsecutiveHopFail += 1
                LastHopFailAt = os.clock()
                task.delay(0.25, function() pcall(function() Hop("post-verify-fail") end) end)
            end
        end)
        return true
    end
    ...
    if ok2 then
        task.delay(Config.PostTeleportVerifyWait, function()
            if g.JobId == beforeJob then
                warn("[TP Fallback] Post-verify fail, sid=", tostring(serverId))
                BadIDs[serverId] = true
                isTeleporting = false
                ConsecutiveHopFail += 1
                LastHopFailAt = os.clock()
                task.delay(0.25, function() pcall(function() Hop("post-verify-fail-fb") end) end)
            end
        end)
        return true
    end
    ...
    while pagesTried < maxPages do
        ...
        if not page or not page.data then
            ...
            for _, c in ipairs(candidates) do
                ...
                local ok, _ = tp_to_instance(PlaceID, sid)
                if not ok then
                    isTeleporting  = false
                    BadIDs[sid]    = true
                    ConsecutiveHopFail += 1
    ...
    TeleportService.LocalPlayerArrivedFromTeleport:Connect(function(loadingGui, data)
        ...
        task.delay(2, function()
            pcall(function()
                ...
                -- reset loot clock để watchdog tính lại từ đầu server mới
                touchLootClock()
            end)
        end)
    end)
...
TeleportService.TeleportInitFailed:Connect(function(_, result, msg)
    ...
    -- Unauthorized / RequestRejected → bỏ hẳn JobId
    if LastAttemptSID then BadIDs[LastAttemptSID] = true end

    isTeleporting = false
    ConsecutiveHopFail += 1
    LastHopFailAt = os.clock()
    task.delay(0.25, function() pcall(function() Hop("TeleportInitFailed") end) end)
end)

-- Sau khi tới server mới, đánh dấu JobId để không quay lại (bảo hiểm nếu executor không bắn ArrivedFromTeleport)
...
local function findUsableChest()
    ...
    for _, v in ipairs(items:GetChildren()) do
        -- chấp nhận đổi tên kiểu khác nhưng vẫn chứa "Chest"
