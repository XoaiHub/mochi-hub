-- ================================================================
-- Mochi Farm (No UI) — Zero-API Hop + Safe-Landing + Loot-Aware
-- Tương thích Wave/PC. Không dùng server list (admin đổi API vẫn chạy).
-- ================================================================

-- ============== CONFIG ==============
local Config = {
    Debug                   = true,     -- bật log để dễ kiểm tra

    -- Farm
    StrongholdChestName     = "Stronghold Diamond Chest",
    StrongholdPromptTime    = 6,
    StrongholdDiamondWait   = 10,
    FarmPlaceId             = 126509999114328,   -- !!! kiểm tra đúng place farm của map bạn
    ...
    -- Chống spam fail khi hop
    MaxConsecutiveHopFail   = 5,
    ConsecutiveHopCooloff   = 6.0,

    -- Zero-API Hop (không dùng server list)
    ZeroAPIHopEnabled       = true,
    BruteHop_MaxAttempts    = 8,      -- thử Teleport(place) tối đa N lần/lượt
    BruteHop_PostVerifySec  = 7.0,    -- chờ sau mỗi lần Teleport để kiểm tra JobId
    ...
    SafeLandingDuration     = 10.0,             -- tối đa giữ trạng thái hạ cánh (giây)
    SafeLandingRayDistance  = 200,
    AntiVoidFollowInterval  = 0.08,
    AntiVoidY               = -40,              -- dưới ngưỡng này coi như rơi
    RecoveryRaise           = 10.0,
    AntiVoidPadSize         = Vector3.new(220, 8, 220),
    AntiVoidPadTransparency = 1,                -- 1: vô hình
...
-- VisitedServers (hạn chế quay lại JobId cũ theo giờ)
...
local function collectAllDiamonds()
    ...
    if not re or not re.FireServer then
        -- không tìm thấy remote mới -> không chặn hop
        return 0
    end

    for _, v in ipairs(items:GetChildren()) do
        if v.Name == "Diamond" then
            pcall(function() re:FireServer(v); count += 1 end)
        end
    end
...
-- ============== NOCLIP (giữ nguyên nhưng tôn trọng Safe-Landing) ==============
getgenv().NoClip = true
...
local function preHopLootDrain()
    ...
    -- nếu thư mục hoặc remote không tồn tại → không chặn hop
    local items = diamondsFolder()
...
-- HOP & HOPFAST (mọi nơi gọi sẽ đi qua guard & safe-landing)
function Hop(reason)
    ...
    while attempts < maxA do
        ...
        while os.clock() - t0 < waitSec do
            if g.JobId ~= startJob then
                log("Arrived in NEW server:", g.JobId)
    ...
    TeleportService.LocalPlayerArrivedFromTeleport:Connect(function()
        ...
        task.delay(2, function()
            pcall(function()
                ...
                isTeleporting      = false
                touchLootClock()
                log("ArrivedFromTeleport handler done")
            end)
        end)
    end)
end)
