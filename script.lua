...
-- - Auto mở Crate khi đủ balls (realtime config, autodetect currency, debug)
...
-- -- tuỳ chọn: getgenv().AutoCrateEnabled = false   -- mặc định true nếu không đặt
...
do
    ...
    LP.CharacterAdded:Connect(function()
        ...
        task.delay(0.5, function()
            -- autoPickPhone đã hook ở phần A
        end)
    end)
...
do
    ...
    -- Map hiển thị -> tham số server (thêm tại đây nếu game đổi tên)
    local AC_BOX_DEFS = {
    ...
    -- ====== Open wrapper: thử nhiều arg để tránh lệch tên currency ======
