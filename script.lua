...
local Config = {
    ...
    FarmTick                = 1.0,
    DiamondTick             = 0.30,   -- nhích nhỏ để hút nhanh hơn
    ...
    -- Chống hop quá sớm
    MinStayAfterJoin        = 8.0,   -- (s) ở lại tối thiểu sau khi vào server
    MinStayAfterFirstGem    = 5.0,   -- (s) ở lại thêm sau khi nhặt viên đầu tiên
    RequirePickupBeforeHop  = true,  -- chỉ Hop nhanh nếu đã nhặt được gems

    -- “No-gems / No-chest” bớt hung hăng
    NoGemsGrace             = 10.0,  -- (s) đợi gems spawn sau join trước khi kết luận no-gems
    NoChestScanWindow       = 15.0,  -- (s) thời gian cho phép scan chest trước khi kết luận no-chest

    -- Farm-first: chắc chắn “hút sạch rồi mới hop”
    FarmHoldAfterSpawn      = 2.3,   -- (s) chờ gems xuất hiện sau khi mở rương
    FarmDrainHold           = 1.8,   -- (s) yêu cầu “hết gems” liên tục ≥ giá trị này
...
ENV.MOCHI._ver = "hop-v2.4-stable"
...
local function http_get(url)
    ...
    if req then
        ...
        if ok2 and res2 then
            if (res2.StatusCode == 200 or res2.Success) and type(res2.Body) == "string" and #res2.Body > 0 then
                return res2.Body
            end
        end
    end
...
local function _onServerEnter()
    Server.joinedAt, Server.firstGemAt, Server.collectedCount = tick(), nil, 0
end
...
local function rotateHour()
    ...
    if G.hour ~= h then
        ...
        for id, ts in pairs(G.ids) do
            if (n - (ts or 0)) > (G.ttl or 10800) then G.ids[id] = nil end
        end
    end
end
local function markVisitedGlobal(id) if id and id ~= "" then rotateHour(); G.ids[id] = now() end end
local function notVisited(id)
    if not id or id == "" then return false end
    if id == g.JobId then return false end
    rotateHour()
    local ts = G.ids[id]
    return not (ts and (now() - ts) <= (G.ttl or 10800))
end

-- Roblox has multiple mirrors; try several
local HOSTS = {
    "https://games.roblox.com",
    "https://games.roproxy.com",
    "https://apis.roproxy.com",
    "https://games.rbxcdn.xyz"  -- thêm 1 mirror đề phòng API đổi
}
...
local function pickServer(placeId)
    ...
    -- Không có ứng viên -> dọn 12% visited cũ để tránh “cạn server”
    if #bestList == 0 then
        ...
        if G._clean_counter % 2 == 0 then
            ...
            for i=1,math.min(cut, #oldest) do G.ids[oldest[i].id] = nil end
            warn("[pickServer] Cleaned", math.min(cut, #oldest), "old visited entries")
        end
    end
...
-- ====== PROXIMITY PROMPT HELPER (BỔ SUNG) ======
local function pressPromptWithTimeout(prox, holdSeconds)
    ...
    while prox and prox.Parent and prox.Enabled and (tick()-t0) <= holdSeconds do
        if typeof(fireproximityprompt) == "function" then
            ...
            -- fallback cho executor không có fireproximityprompt
            local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
            if hum and prox.HoldDuration == 0 then
                hum:MoveTo(prox.Parent and prox.Parent.Position or HRP().Position)
            end
        end
        task.wait(0.25)
...
-- Chống rớt map sau teleport (đệm an toàn)
...
local function tpToJob(jobId)
    ...
    if not ok then
        ...
        if not ok2 then task.wait(1.2); STATE.BusyTeleport=false; return false end
    end
...
-- Collect diamonds (đếm pickup để guard hop, tự nhích lại gần nếu xa)
...
-- Hút gems đến khi “hết hẳn” liên tục ≥ FarmDrainHold
        ...
        if got == 0 then
            ...
            if items then
                ...
                if near and nd > 70 and r then
                    ...
                    task.wait(0.12)
                    got = collectAllDiamonds(); total += got
                end
            end
        end
...
-- ===== NO-GEMS WATCHER (không đẩy đi nếu còn rương mở được) =====
...
-- Compatibility mark-visited safeguard
