...
local function safeSetCFrame(partCFrame)
    if not (lp and lp.Character and lp.Character.PrimaryPart) then
        return
    end
    pcall(function()
...
local function checkTeleporter(obj)
    ...
    if not (g and g:FindFirstChild('BillboardGui') and g.BillboardGui:FindFirstChild('Players')) then
        return
    end
    ...
    if not (x and y and x >= 2) then
        return
    end
    local enter = obj:FindFirstChildWhichIsA('BasePart')
    if not (enter and lp and lp.Character and lp.Character:FindFirstChild('Humanoid')) then
        return
    end
    ...
    while task.wait(0.5) do
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj:IsA('Model') then
                local n = obj.Name
                if n == 'Teleporter1' or n == 'Teleporter2' or n == 'Teleporter3' then
                    pcall(function()
                        checkTeleporter(obj)
                    end)
                end
...
if workspace:FindFirstChild('Map') and workspace.Map:FindFirstChild('Boundaries') and workspace.Map.Boundaries:FindFirstChild('Fog') then
    local ok, bb = pcall(function()
        return workspace.Map.Boundaries.Fog:GetBoundingBox()
    end)
    if ok and bb then
    ...
    while task.wait(0.1) do
        if game.PlaceId == 126509999114328 then
            ...
            while true do
                ...
                if os.time() - startPrompt >= 10 then
                    -- skip cai nay di
                end
                task.wait(0.5)
            end
