...
local Config = {
    ...
    StrongholdPromptTime    = 6,     -- tối đa giữ prompt để mở Stronghold
    StrongholdDiamondWait   = 10,    -- chờ diamond spawn sau khi mở
...
-- ===== Stronghold helpers (FIX đứng im) =====
local function approachModelPivot(model, yOffsets)
    local pv = model:GetPivot()
    local ok = false
    for _, dy in ipairs(yOffsets or {3, 1.8, 4.5}) do
        pcall(function() tpCFrame(CFrame.new(pv.Position + Vector3.new(0, dy, 0))) end)
        task.wait(0.15)
        local r = HRP()
        if r and (r.Position - pv.Position).Magnitude <= 8 then ok = true; break end
    end
    return ok
end

local function holdPromptWithWatchdog(prompt, maxSeconds, step)
    local t0 = os.clock()
    local stepDur = step or 0.25
    maxSeconds = maxSeconds or 6
    while prompt and prompt.Parent and (os.clock() - t0) < maxSeconds do
...
local function openStrongholdOrHop()
    ...
    approachModelPivot(sh, {3, 1.8, 4.5})
    local opened = holdPromptWithWatchdog(prox, Config.StrongholdPromptTime, 0.28)
    prox = findProximityPromptInChest(sh)

    if not opened and prox and prox.Parent and prox.Enabled then
        strongholdTried[sid] = true
        HopFast("locked:stronghold-stall")
        return
    end
    ...
    if waitDiamonds(Config.StrongholdDiamondWait) then
        ...
        strongholdTried[sid] = true
        HopFast("locked:stronghold-no-diamonds")
    end
end
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            -- 1) STRONGHOLD (mọi case kẹt/không mở/không spawn → hop ngay)
            openStrongholdOrHop()
            ...
            if not chest then
                ...
                if okOpen then
                    ...
                    chestTried[id] = true
                    HopFast("chest-locked") -- bypass grace
                end
            end
        end
    end
