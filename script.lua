...
local Config = {
    ...
    StrongholdPromptTime  = 6,   -- thử mở Stronghold tối đa 6 giây
    StrongholdDiamondWait = 10,  -- chờ diamond spawn tối đa 10 giây
...
spawn(function()
    while task.wait(1) do
        ...
        if stronghold then
            local sid = stronghold:GetDebugId()
        ...
        while true do
            ...
            if chestTried[id] then
                warn("[ChestFarm] Chest fail trước đó -> skip")
            ...
            if success then
                ...
                chestTried[id] = true
                warn("[ChestFarm] Chest kẹt -> đánh dấu skip")
            end
            task.wait(0.5)
        end
    end
