...
--[[ ====== BEGIN MINIMAL BUY-ONLY FIX (merged) ====== ]]
--[[
===============================================================================
MINIMAL BUY-ONLY FIX (Drop-in)
- Keeps ALL existing features and behaviors intact.
- Only improves reliability of BUY by:
  * Robust root-part detection (HumanoidRootPart/RootPart/BasePart)
  * Reliable proximity prompt activation (hold + fireproximityprompt fallback)
  * Gentle pathing/teleport close to target before prompt
- Does NOT modify AutoLock, SELL, COLLECT, Webhook, AntiBan, etc.
===============================================================================
]]
...
local function _approachCFrame(cf)
    -- Prefer PathfindTo2 if available (from original script)
    if typeof(PathfindTo2) == "function" then
        return PathfindTo2(cf.Position or cf)
    end
    -- Fallback: snap HRP close (minimal interference)
    waitForCharacter()
...
do
    ...
    BuyAnimal = function(Animal, Info)
        ...
        if not CurrentBuyingAnimal then
            CurrentBuyingAnimal = Info
            _approachCFrame(rootPart.CFrame + rootPart.CFrame.LookVector * 31)
            _tryPrompt(prompt)
...
do
    ...
    TaskBuyAnimal = function(data)
        -- re-use original flow but more robust to prompt/path issues
        ...
        local CurrentCash = cashVal and cashVal.Value or 0

        local initialCash = CurrentCash
        local Passed = False or false
        repeat
            leaderstats = Players.LocalPlayer:FindFirstChild("leaderstats")
            cashVal = leaderstats and leaderstats:FindFirstChild("Cash")
            CurrentCash = cashVal and cashVal.Value or CurrentCash
            ...
            if typeof(isStorageFull) == "function" and isStorageFull() then
                local getAnimalsToSellFn = (typeof(getAnimalsToSell) == "function") and getAnimalsToSell or nil
                local calcValueFn = (typeof(calculateTotalValue) == "function") and calculateTotalValue or nil
                local sellableAnimals = getAnimalsToSellFn and getAnimalsToSellFn(data.Info) or {}
                if #sellableAnimals == 0 then break end
                local targetValue = calcValueFn and calcValueFn(data.Info) or math.huge
            ...
            repeat
                ...
                if hrp then
                    ...
                    if (hrp.Position - rootPart.Position).Magnitude > 6 then
                        _approachCFrame(rootPart.CFrame)
                    end
                end
        ...
        until ((Players.LocalPlayer:FindFirstChild("leaderstats")
                and Players.LocalPlayer.leaderstats:FindFirstChild("Cash")
                and Players.LocalPlayer.leaderstats.Cash.Value or 0) < initialCash)
    end
end

--[[ ====== END MINIMAL BUY-ONLY FIX ====== ]]
