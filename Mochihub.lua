        ...
        if not t.complete then
            ...
            if isTreatFeed and t.remaining > 0 then
                ...
                if have >= t.remaining then
                    ...
                    HiveManager.Feed(bee.X, bee.Y, "Treat", t.remaining, false)
                    task.wait(1)
                    return true, false
                end
                return false, true
...
function Stats.getItemCount(itemName)
    if Stats._cachedStats then
        local s = Stats._cachedStats
        local v = (s.Eggs and s.Eggs[itemName]) or (s.Items and s.Items[itemName])
