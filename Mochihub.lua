...
function Tween.moveToField(fieldName)
    ...
    if field then
        ...
        if heightOffset > 0 then
            ...
            Tween.tweenTo(highCFrame)
            task.wait(0.3)
            Tween.tweenTo(field.CFrame + Vector3.new(0, 5, 0))
        else
...
function Gear.tryUpgradeTool()
    ...
    if available >= nextTool.cost then
        ...
        -- Check if honey decreased = purchase succeeded
        ...
        if honeyAfter < honeyBefore then
            ...
            Gear._equippedTool = nextTool.name  -- PurchaseEffect auto-equips
            print("[Gear] Tool upgraded to: " .. nextTool.name)
            return true
        else
            -- Honey didn't decrease = already owned, mark and skip to next
            print("[Gear] Already own tool (honey unchanged): " .. nextTool.name .. " -> skipping")
            table.insert(Gear._ownedTools, nextTool.name)
            return true  -- return true so while loop tries next
        end
    end
        ...
        if not t.complete and t.remaining > 0 then
            ...
            for _, item in ipairs(items) do
                if string.find(desc, item.match) or string.find(typ, item.match) then
                    ...
                    print("[Feed] Quest needs " .. item.name .. " | Have: " .. have .. " (key=" .. item.remote .. ") | Need: " .. t.remaining)
                    if have > 0 then
                        local amount = math.min(have, t.remaining)
                        local bees = HiveManager.GetBee()
                        if #bees > 0 then
                            local bee = bees[math.random(1, #bees)]
                            print("[Feed] Feeding " .. amount .. " " .. item.name .. " at [" .. bee.X .. "," .. bee.Y .. "]")
                ...
                if isFeedTask and not isTreat and not isRJ then
                    for _, item in ipairs(items) do
                        ...
                        if have > 0 then
                            ...
                            if #bees > 0 then
                                local bee = bees[math.random(1, #bees)]
                                print("[Feed] Generic feed: using " .. amount .. " " .. item.name .. " at [" .. bee.X .. "," .. bee.Y .. "]")
                                HiveManager.Feed(bee.X, bee.Y, item.remote, amount)
                                task.wait(1)
                                return true
                            end
...
function Quest.doFarmLoop(fieldName, duration)
    ...
    while tick() - loopStart < duration do
        ...
        elseif Config.EnableMobAvoidance and MobManager.isMobNearby() then
            ...
            local tokensInField = TokenFolder[fieldName]
            if tokensInField and next(tokensInField) then
                local _, best = TokenManager.tokenMinimax(tokensInField, {
                    CollectedToken = {},
                    Speed = plr.Character and plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.WalkSpeed or 16,
                    Position = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") and plr.Character.HumanoidRootPart.Position or Vector3.new(0, 0, 0),
                    Point = 0
                }, 3)
                if best then
                    TokenManager.moveToToken(best, tokensInField)
                end
            else
                local rndPos = TokenManager.getRandomFieldPosition(fieldName)
                if rndPos then
                    TokenManager.smartMove(rndPos)
                    task.wait(0.3)
                end
            end
...
function SellManager.sell()
    ...
    local atHive = pcall(function()
        return plr:DistanceFromCharacter(hivePos) < 5
    end)
    if not atHive then
        SellManager.tpToHive()
        task.wait(2)
...
function HiveManager.useStarJelly()
    ...
    print("[StarJelly] Using on " .. target.Type .. " (" .. target.Rarity .. ") at [" .. target.X .. "," .. target.Y .. "] | Have: " .. starJellyCount)
...
function TokenManager.smartMove(targetPos)
    ...
    if success and path.Status == Enum.PathStatus.Success then
        ...
        for i, waypoint in ipairs(waypoints) do
            ...
            repeat
                Run.Heartbeat:Wait()
            until (root.Position - waypoint.Position).Magnitude < 4 or (tick() - startWait) > 1
        end
    else
...
function TokenManager.moveToToken(Token, TokenList)
    if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
        local StartWalking = tick()
        local root = plr.Character.HumanoidRootPart
        if (root.Position - Token.Position).Magnitude > 15 then
            TokenManager.smartMove(Token.Position)
        else
        ...
        repeat
            task.wait()
        until not TokenList[Token.ID] or (tick() - StartWalking > 5) or (root.Position - Token.Position).Magnitude < 3

        TokenList[Token.ID] = nil
    end
end

function TokenManager.tokenMinimax(ListAllToken, CurrentState, Depth)
    if Depth == 0 or #ListAllToken == 0 then
