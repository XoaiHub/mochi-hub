...
-- Gợi ý: nếu bạn load script này qua Luarmor Loader (Init.lua),
-- hãy truyền lại “loader code” của bạn vào đây để auto chạy lại sau teleport.
-- Ví dụ:
-- queue_on_teleport_compat([[loadstring(game:HttpGet("https://yourdomain.com/Init.lua"))()]])
-- Hoặc nếu bạn dùng raw GitHub:
-- queue_on_teleport_compat([[loadstring(game:HttpGet("https://raw.githubusercontent.com/XoaiHub/mochi-hub/refs/heads/master/Kaitun%2099Night.lua"))()]])
    ...
    for _, v in ipairs(page.data) do
        ...
        if playing and maxp and playing < maxp then
            if not hasValue(AllIDs, sid) and regionMatch(v) then
                ...
                isTeleporting = true
                resetState()
                task.wait(rand(Config.HopBackoffMin, Config.HopBackoffMax)) -- backoff nhẹ
...
local function autoJoinOrCreate()
    -- Ưu tiên join room có người (>=2), nếu không có thì tự đứng vào teleporter để create
    local joined = false
    ...
    if not joined then
        -- Không có room sẵn: tự "create" bằng cách đứng vào teleporter trống
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj:IsA("Model") and (obj.Name=="Teleporter1" or obj.Name=="Teleporter2" or obj.Name=="Teleporter3") then
                local enter = obj:FindFirstChildWhichIsA("BasePart")
                if enter and HRP() then
