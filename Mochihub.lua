...
-- + Fast Hop After Collect (nhảy ngay sau khi NHẶT XONG gems)
-- + Anti-DEAD + Skip Server Full + No-Rejoin-Same-Server
-- + Hop engine (Asc/Desc + cursor + region + tránh trùng) đã gia cố
...
local Config = {
    ...
    StrongholdPromptTime    = 6,    -- thời gian cố gắng giữ prompt stronghold
    StrongholdDiamondWait   = 10,   -- chờ diamond spawn sau khi mở stronghold
    ...
    HopAfterStronghold      = true, -- hop sau khi NHẶT xong stronghold
    HopAfterNormalChest     = true, -- hop sau khi NHẶT xong chest thường
    HopPostDelay            = 0.20,
...
-- ===== HOP ENGINE (kiểu classic + gia cố) =====
...
local function hookTeleportErrorPrompt()
    ...
    pcall(function()
        local overlay = game.CoreGui:WaitForChild("RobloxPromptGui",5) and game.CoreGui.RobloxPromptGui:FindChildWhichIsA("Frame", true)
        if overlay and overlay.Parent and overlay.Parent:FindFirstChild("promptOverlay") then
            local pl = overlay.Parent.promptOverlay
            for _,v in pairs(pl:GetChildren()) do bQ(v) end
            pl.ChildAdded:Connect(bQ)
        end
    end)
end
    ...
    for _, v in ipairs(items:GetChildren()) do
        if v.Name == "Diamond" then
            pcall(function()
                ...
                local re = ev and ev:FindChildWhichIsA("RemoteEvent", true)
                re = (ev and ev:FindFirstChild("RequestTakeDiamonds")) or re
...
local function findStrongholdChest()
    local items = workspace:FindChildWhichIsA("Folder") and workspace:FindFirstChild("Items")
    if not items then items = workspace:FindFirstChild("Items") end
...
-- ===== MAIN FARM (đã fix: NHẶT XONG mới hop; khóa/timeout/no-spawn → hop) =====
task.spawn(function()
    while task.wait(Config.FarmTick) do
        ...
        if sh then
            ...
            if not strongholdTried[sid] then
                ...
                -- Khóa / không có prompt → hop ngay
                if not (prox and prox.Enabled) then
                    strongholdTried[sid] = true
                    warn("[Stronghold] Locked / no prompt → hop")
                    Hop("Low")
                else
                    ...
                    if opened and not (prox and prox.Parent and prox.Enabled) then
                        ...
                        if not seen then
                            strongholdTried[sid] = true
                            warn("[Stronghold] Opened but no diamonds spawned → hop")
                            Hop("Low")
                        else
                            ...
                            if got > 0 or after < before or after == 0 then
                                ...
                                strongholdTried[sid] = true
                                warn("[Stronghold] Diamonds not collectible → hop")
                                Hop("Low")
                            end
                        end
                    else
                        -- Timeout / vẫn khóa → hop
                        strongholdTried[sid] = true
                        warn("[Stronghold] Prompt timeout/locked → hop")
                        Hop("Low")
                    end
                end
            end
        ...
        -- 2) Chest thường (giữ đúng luật: nhặt xong mới hop; khóa/timeout → hop)
        ...
        if not chest then
            ...
            if not (prox and prox.Enabled) then
                chestTried[id] = true
                warn("[Chest] Locked / no prompt → hop")
                Hop("Low")
            else
                local t0   = os.clock()
                local okOpen = false
                ...
                if okOpen then
                    ...
                    if got > 0 or after < before or after == 0 then
                        ...
                        chestTried[id] = true
                        warn("[Chest] Diamonds not collectible / none spawned → hop")
                        Hop("Low")
                    end
                else
                    chestTried[id] = true
                    warn("[Chest] Prompt timeout / cannot open → hop")
                    Hop("Low")
                end
            end
        end
