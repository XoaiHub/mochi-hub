...
local function getTeleporterCount(teleporter)
    local count = 0
    for _, player in pairs(Players:GetPlayers()) do
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            local pos = player.Character.HumanoidRootPart.Position
            if (pos - teleporter.Position).Magnitude < 5 then -- khoảng cách để tính là đứng trên teleporter
                count = count + 1
            end
        end
    end
...
-- Lấy teleporter dựa trên index
local teleporter = workspace.Teleporter3:GetChildren()[getgenv().TeleporterIndex]
