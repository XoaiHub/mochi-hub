...
local function TaskBuyAnimal(data)
    ...
    -- Early exit if lock is expiring soon; pause & requeue
    if getgenv().Configcuttay and getgenv().Configcuttay["Settings"]["Lock Base"] and lockExpiresSoon(0) then
        if typeof(pauseBuyTasks) == "function" then pauseBuyTasks() end
        local retry = createTask("BUY", TaskManager.priorities.BUY, TaskBuyAnimal, data)
        retry.cancelled = false
        table.insert(TaskManager.pausedTasks, retry)
        processTasks()
        return
    end
    ...
    if AnimalInfo then
        if getgenv().Configcuttay["Settings"]["Auto Buy Animals"] then
            ...
            if (isDesiredRarity or isRebirthRequired) and canAfford and shouldBuy then
                ...
                -- LockBase gating: only enqueue BUY when lock window is safe
                if getgenv().Configcuttay and getgenv().Configcuttay["Settings"]["Lock Base"] then
                    if not isLockActive() then
                        return
                    end
                    if lockExpiresSoon(0) then
                        return
                    end
                end
...
-- === LOCK HELPERS (AUTO-INSERTED) ===
local function isLockActive()
    return (getLockRemainingTime() or 0) > 0
end

local function lockExpiresSoon(extraBuffer)
    local remain = getLockRemainingTime() or 0
    local tpNeed = (GetTPToLockTime() or 5) + 6
    local buf = tonumber(extraBuffer) or 0
    return (remain > 0) and (remain <= (tpNeed + buf))
end
...
local function PreLock()
    ...
    -- Nếu đang lock và còn đủ thời gian -> cho BUY chạy bình thường (resume nếu đang pause)
    if isLockActive() and not lockExpiresSoon(0) then
        if #TaskManager.pausedTasks > 0 then
            if typeof(resumeBuyTasks) == "function" then
                resumeBuyTasks()
            end
            processTasks()
        end
    ...
    -- Hết/sắp hết lock -> ưu tiên lock
    local remain = getLockRemainingTime()
    local need = (GetTPToLockTime() or 5) + 6
    if (remain == 0) or (remain <= need) then
        if TPingTP then return end
        TPingTP = true
        if getgenv().Configcuttay["Settings"]["Lock Priority"] then
            cancelTasks("SELL")
            cancelTasks("BUY")
