...
-- Local tracking of purchased gear (stats cache is unreliable)
...
function Gear.getCurrentGear()
    ...
    -- Use stats for equipped info, fallback to local tracking
    local tool = (stats and stats.EquippedCollector) or Gear._equippedTool or "Scooper"
    local bag = (stats and stats.EquippedBackpack) or Gear._equippedBag or "Pouch"
...
function Gear.tryUpgradeTool()
    ...
    if available >= nextTool.cost then
        ...
        if honeyAfter < honeyBefore then
            ...
            warn("[Gear] Tool purchase failed (honey unchanged) for: " .. nextTool.name)
        end
    end
...
function Gear.tryUpgradeBackpack()
    ...
    if available >= nextBag.cost then
        ...
        local honeyAfter = getHoney()
        if honeyAfter < honeyBefore then
            print("[Gear] Bag purchased! Honey: " .. honeyBefore .. " -> " .. honeyAfter)
            table.insert(Gear._ownedBags, nextBag.name)
            Gear._equippedBag = nextBag.name  -- PurchaseEffect auto-equips
            print("[Gear] Bag upgraded to: " .. nextBag.name)
            return true
        else
            warn("[Gear] Bag purchase failed (honey unchanged) for: " .. nextBag.name)
        end
    end
    ...
    for _, t in ipairs(progress.tasks) do
        if not t.complete then
            ...
            if (string.find(typ, "use items") or string.find(typ, "feed")) and t.remaining > 0 then
                local desc = string.lower(t.description)
                local items = {"Strawberry", "Blueberry", "Pineapple", "Sunflower Seed", "Moon Charm"}
    ...
    for _, t in ipairs(progress.tasks) do
        if not t.complete and t.remaining > 0 then
            ...
            local items = {
                ...
                {name = "Sunflower Seed", remote = "Sunflower Seed"},
