-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL (v9.3)
...
-- + NEW: Watchdog không-tiến-trình + KeepAlive cứng + bỏ qua grace khi "no-chest"
...
local Config = {
    ...
    -- ====== NEW: Bảo đảm hop/nhặt liên tục ======
    -- Nếu X giây không mở rương cũng không nhặt thêm kim cương -> hop ngay
    ForceHopIfNoProgress    = 25,
    -- Dù có tiến trình, ở cùng một server quá Y giây cũng buộc hop (chống "đơ map")
    MaxServerStaySeconds    = 120,
    -- Hết hạn đánh dấu BadIDs/Visited (an toàn)
...
local function HopClassic(mode, reason)
    ...
    local bypassReasons = {
        ...
        ["locked"]=true, ["no-chest"]=true, ["no-progress"]=true, ["keepalive"]=true, ["scan"]=true
    }
    if not bypassReasons[tostring(reason or "manual")] then
...
local function Hop(reason_or_mode)
    -- Mặc định cho phép bypass grace bằng "scan"
...
local function HopFast(reason)
    ...
    -- đảm bảo không kẹt cờ HopRequested
...
-- Mark visited & mốc thời gian ở server
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            -- 2) Chest thường (KHÔNG mở được -> Hop ngay; KHÔNG thấy rương -> hop bypass grace)
...
-- ===== NEW: WATCHDOG không-tiến-trình + KEEPALIVE buộc hop =====
task.spawn(function()
    while task.wait(1.0) do
        if g.PlaceId == Config.FarmPlaceId then
            -- buộc hop nếu quá lâu không có tiến trình (mở rương/nhặt kim cương)
            if secondsSinceProgress() >= Config.ForceHopIfNoProgress and not isTeleporting then
                HopFast("no-progress")
            end
            -- buộc hop nếu ở cùng server quá lâu (kể cả có tiến trình) để đảm bảo vòng lặp
