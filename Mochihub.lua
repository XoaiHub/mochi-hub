-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL (Fix Stronghold stall)
-- + Fast Hop After Collect
-- + Stronghold V2: 6s không mở -> Hop; mở -> hút sạch -> Hop (không bao giờ kẹt)
-- + Anti-DEAD + Skip Server Full + No-Rejoin-Same-Server
-- + Hop engine: Asc/Desc + cursor (gia cố) + guard + grace windows (bypass khi locked)
...
local Config = {
    ...
    StrongholdPromptTime    = 6,      -- GIỚI HẠN 6s
...
-- ===== Stronghold V2: tuyệt đối không kẹt =====
local STRONGHOLD_OP_START = 0

local function approachModelPivot(model, yOffsets)
    local pv = model:GetPivot()
    for _, dy in ipairs(yOffsets or {3.0, 1.8, 4.5}) do
        pcall(function() tpCFrame(CFrame.new(pv.Position + Vector3.new(0, dy, 0))) end)
        task.wait(0.14)
        local r = HRP()
        if r and (r.Position - pv.Position).Magnitude <= 8 then return true end
    end
    return false
end

local function holdPromptUntilGone(prompt, maxSeconds, step)
    local t0 = os.clock()
    local dur = maxSeconds or 6
    local stp = step or 0.25
    while (os.clock() - t0) < dur do
        if not (prompt and prompt.Parent) then return true end
        ...
        if typeof(fireproximityprompt) == "function" then
            ...
            pcall(function()
                ...
                if hum and prompt.HoldDuration == 0 then hum:MoveTo(prompt.Parent.Position) end
            end)
        end
        task.wait(stp)
    end
...
local function waitDiamondsStrict(limit)
    local t0 = os.clock()
    local lim = limit or Config.StrongholdDiamondWait
...
local function drainAllDiamonds()
    local lastGot = os.clock()
    while true do
        local got = collectAllDiamonds()
        if got > 0 then lastGot = os.clock() end
        if (os.clock() - lastGot) >= 0.8 and not diamondsLeft() then break end
        task.wait(0.12)
    end
end

local function strongholdControllerV2()
    local sh = findStrongholdChest()
    if not sh then return end

    local sid = sh:GetDebugId()
    if strongholdTried[sid] then return end

    local prox = findProximityPromptInChest(sh)
    if not (prox and prox.Enabled) then
        strongholdTried[sid] = true
        HopFast("locked:stronghold-no-prompt")
        return
    end

    approachModelPivot(sh, {3.0, 1.8, 4.5})
    STRONGHOLD_OP_START = os.clock()

    local opened = holdPromptUntilGone(prox, Config.StrongholdPromptTime, 0.28)
    prox = findProximityPromptInChest(sh)

    -- HẾT 6s mà vẫn còn prompt Enabled => coi như không mở được
    if not opened and prox and prox.Parent and prox.Enabled then
        strongholdTried[sid] = true
        HopFast("locked:stronghold-timeout-6s")
        return
    end

    -- ĐÃ MỞ: phải có diamonds, nếu không => hop
    if not waitDiamondsStrict(Config.StrongholdDiamondWait) then
        strongholdTried[sid] = true
        HopFast("stronghold-opened-no-diamonds")
        return
    end
...
-- Watchdog phụ: >12s thao tác Stronghold mà chưa xong => hop
task.spawn(function()
    while task.wait(0.5) do
        if STRONGHOLD_OP_START > 0 and (os.clock() - STRONGHOLD_OP_START) > 12 then
            STRONGHOLD_OP_START = 0
            HopFast("watchdog:stronghold-stall")
        end
    end
end)
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            -- 1) STRONGHOLD (6s không mở -> Hop; mở -> hút sạch -> Hop)
            strongholdControllerV2()

            -- 2) Chest thường (không mở -> Hop; mở -> nhặt -> Hop)
            ...
            if not chest then
                ...
                while prox and prox.Parent and prox.Enabled and (os.clock() - t0) < 8 do
